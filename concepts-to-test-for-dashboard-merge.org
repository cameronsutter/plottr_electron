...
** DONE Update notification styles are broken
** DONE Overflow style of dashboard is broken
   Add many files.  You can't open the bottom ones.
** DONE Opening a link from beamer no longer works
** DONE Beamer bell looks out of place on new dashboard
   I Removed the bell.
...
** DONE Open a file after just saving as temp and it'll open it in a new window :'(
** DONE The first time that I tried to save a new file it didn't prompt me
...
** DONE Removing an item from the list doesn't remove it from all dashboards

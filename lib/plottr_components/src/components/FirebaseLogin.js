...
const FirebaseLoginConnector = (connector) => {
  const {
    platform: {
      log,
      firebase: { startUI, firebaseUI, onSessionChange, fetchFiles, currentUser },
      license: { checkForPro },
      isDevelopment,
    },
  } = connector
  const SETTINGS = connector.platform.settings
  checkDependencies({ log, startUI, firebaseUI, onSessionChange, fetchFiles, currentUser })

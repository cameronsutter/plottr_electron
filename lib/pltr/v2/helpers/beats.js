...
export function idOfBeatBeforeWhenReordered(items, toMove, droppedOntoId) {
  const newTree = moveNextToSibling(cloneDeep(items), toMove, droppedOntoId)
  const sortedBeats = beatsByPosition(() => true)(newTree)
  const indexOfBeatBefore = sortedBeats.findIndex(({ id }) => id === toMove)
  // TODO: handle inserting into position 0
  const beatBeforeId = indexOfBeatBefore <= 0 ? null : sortedBeats[indexOfBeatBefore - 1].id
  return beatBeforeId
}

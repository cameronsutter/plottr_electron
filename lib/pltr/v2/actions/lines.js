...
import { pinnedPlotlinesSelector } from '../selectors/ui'
...
export const reorderLines = (droppedPosition, originalPosition, lines, bookId) => (dispatch) => {
  const isConflictWithPinned = lines.find(
    (line) => line.position && line.isPinned && line.position == droppedPosition
  )
  const isPinnedPlotline = lines.find(
    (line) => line.position && line.isPinned && line.position == originalPosition
  )

  const bothPinned = lines.find(
    (line) =>
      line.isPinned &&
      line.position &&
  ...
  if (!bothPinned && isPinnedPlotline) {
    return false
  }

  const newPosition = isConflictWithPinned && !bothPinned ? droppedPosition + 1 : droppedPosition
  const reorderedLines = reorderList(newPosition, originalPosition, lines)
  return dispatch({ type: REORDER_LINES, lines: reorderedLines, bookId })
}

export const togglePinPlotline = (line, lines, bookId) => (dispatch, getState) => {
  ...
  if (line.isPinned) {
    ...
    const reorderedLines = reorderList(0, line.position, lines)
    return dispatch({ type: PIN_PLOTLINE, lineId: line.id, lines: reorderedLines, bookId })
  }
}

export function unPinPlotline(lineId, lines, bookId) {
  return { type: UNPIN_PLOTLINE, lineId, lines, bookId }
}

...
describe('normalize', () => {
  ...
  describe('given an empty collection of chilrden, e.g. an empty paragraph', () => {
    ...
    it("shouldn't transform them into lists mistakenly", () => {
      const actualFixForEmptyParagraphs = [
        {
          type: 'paragraph',
          children: [
            {
      ...
      expect(normalize(dummyLog)(exampleEmptyParagraph)).toEqual(actualFixForEmptyParagraphs)
    })
  })
  describe('given a collection of list items', () => {
    describe("that aren't in a list", () => {
      describe('and are all empty', () => {
        const sample1 = [
          {
            type: 'list-item',
            children: [
        ...
        it('should change the node type to paragraph', () => {
          const fixed1 = [
            {
              type: 'paragraph',
              children: [
          ...
          expect(normalize(dummyLog)(sample1)).toEqual(fixed1)
          const fixed2 = [
            {
              type: 'paragraph',
              children: [
          ...
          expect(normalize(dummyLog)(sample2)).toEqual(fixed2)
        })
      })
    })
  })

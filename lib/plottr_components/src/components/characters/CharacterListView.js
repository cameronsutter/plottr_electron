...
const CharacterListViewConnector = (connector) => {
  ...
  const CharacterListView = ({
    ...
    const renderVisibleCharacters = (categoryId) => {
      ...
      if (!!attributeTabId && attributeTabId !== 'all') {
        const filterByBookCharactersByCategory = visibleCharactersByCategory[categoryId].filter(
          (ch) => ch.bookIds.includes(attributeTabId)
        )

        return filterByBookCharactersByCategory.map((ch) => (
          <CharacterItem
            key={ch.id}
            character={ch}

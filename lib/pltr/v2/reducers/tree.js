...
export const maxDepth = (idProp) => (tree) => {
  reduce(idProp)(
    tree,
    (acc, next) => {
      return Math.max(acc, tree.depth(tree, next.id))
    },
    0
  )
}

export function nextPosition(beatTree, parent) {
  return (
    children(beatTree, parent || null).reduce(
...
export const length = ({ index }) => {
  return Object.values(index).length
}

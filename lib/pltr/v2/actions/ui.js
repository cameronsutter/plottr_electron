...
import {
  ...
  SET_BOOK_DIALOG_OPEN,
  SET_BOOK_DIALOG_CLOSE,
} from '../constants/ActionTypes'
import { allBooksSelector } from '../selectors/books'
...
export const setBookDialogOpen = (bookId, bookNumber) => (dispatch, getState) => {
  // NOTE: Mobile doesn't use history middleware
  const fullState = getState()
  const state = fullState.present ? fullState.present : fullState
  const allBooks = allBooksSelector(state)
  const bookExists = mapValues(allBooks, (book) => book.id == bookId)
  if (bookExists) {
    dispatch({
      type: SET_BOOK_DIALOG_OPEN,
      bookId,
      bookNumber,
    })
  }
...
export function setBookDialogClose() {
  return { type: SET_BOOK_DIALOG_CLOSE }
}

...
const TagListViewConnector = (connector) => {
  ...
  const TagListView = ({ tags, actions, darkMode, tagsByCategorySelector, categories }) => {
    ...
    useEffect(() => {
      setTagDetailId(detailID(tagsByCategorySelector, tags, categories, tagDetailId))
    }, [tags, tagsByCategorySelector, categories])
    ...
    const renderTags = () => {
      ...
      return categories.map((cat) => renderCategory(cat))
    }

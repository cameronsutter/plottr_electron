...
export default function ProjectTemplateDetails ({ template }) {
  ...
  const renderData = (type, data) => {
    switch (type) {
      ...
      case 'notes':
        return sortBy(data, 'lastEdited').reverse().map(n => <li key={n.id}>{n.title}</li>)
      case 'charactersCA':
        return data.map(attr => <li key={attr.name}>{attr.name}: {attr.type}</li>)
      default:
        return null
    }
  }
  ...
  const arrayObjects = ['chapters', 'cards', 'lines', 'notes', 'tags']
    ...
    .map(heading => {
      ...
      return <div key={heading}>
        ...
        <ol>
          { renderData(heading, templateData[heading]) }
        </ol>
      </div>
    })
  ...
  const customAttributes = Object.keys(templateData.customAttributes)
    ...
    .map(type => {
      ...
      return <div key={type}>
        <p>{headingText}</p>
        <ol>
          { renderData(`${type}CA`, templateData.customAttributes[type]) }
        </ol>
      </div>
    })

  const images = Object.keys(templateData.images)
    .map(id => {
      const image = templateData.images[`${id}`]

      return <div key={id} style={{width: '50%'}}>
        <p>{image.name}</p>
  ...
  return <div className='panel-body'>
    { arrayObjects }
    { customAttributes.length ? <h5 className='text-center text-capitalize'>{i18n('Custom Attributes')}</h5> : null }
    { customAttributes }

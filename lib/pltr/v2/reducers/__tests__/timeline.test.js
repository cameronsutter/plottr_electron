    ...
    describe('and the add action type', () => {
      describe('and the book id 1', () => {
        it('should enclose the level 1 of the hierarchy in the same parent and leave others alone', () => {
          ...
          const hierarchyForBook1 = newTimelineState.hierarchy.find(({ bookId }) => bookId === '1')
          const hierarchyForBook2 = newTimelineState.hierarchy.find(({ bookId }) => bookId === '2')
          const hierarchyForBookSeries = newTimelineState.hierarchy.find(
            ({ bookId }) => bookId === 'series'
          )
          // Leaves the others alone
          expect(hierarchyForBook2).toEqual(threeLevelHierarchyState.hierarchy[2])
          expect(hierarchyForBookSeries).toEqual(threeLevelHierarchyState.hierarchy[1])
          // Wraps the children of 1
          expect(hierarchyForBook1).toHaveLength(1)
          expect(hierarchyForBook1[0].level).toEqual(3)
          expect(hierarchyForBook1[0].children).toHaveLength(1)
          expect(hierarchyForBook1[0].children[0]).toEqual(threeLevelHierarchyState[0].hierarchy[0])
        })
      })
      describe('and the book id 2', () => {
        it('should enclose the level 1 of the hierarchy in the same parent', () => {
          ...
          // Leaves the others alone
          expect(hierarchyForBook1).toEqual(threeLevelHierarchyState.hierarchy[0])
          expect(hierarchyForBookSeries).toEqual(threeLevelHierarchyState.hierarchy[1])
          // Wraps the children of 1
          expect(hierarchyForBook2).toHaveLength(1)
          expect(hierarchyForBook2[2].level).toEqual(2)
          expect(hierarchyForBook2[2].children).toHaveLength(1)
          expect(hierarchyForBook2[2].children[0]).toEqual(threeLevelHierarchyState[0].hierarchy[0])
        })
      })
    })
    describe('and the delete action type', () => {

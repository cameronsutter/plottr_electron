...
const CardDialogConnector = (connector) => {
  ...
  const CardDialog = ({
    ...
    const newAttributeInputRef = React.createRef()
    ...
    const componentDidUpdate = (prevProps, prevState) => {
      // TODO!!!
      if (newAttributeInputRef.current) newAttributeInputRef.current.focus()
    }
    ...
    const handleNewAttributeTypeChange = (event) => {
      setNewAttributeType(event.target.checked ? 'paragraph' : 'text')
    }

    const addNewCustomAttribute = (name) => {
      if (name) {
        customAttributeActions.addCardAttr({
          name,
    ...
    const closeNewAttributeSection = () => {
      setAddingAttribute(false)
      setNewAttributeType('text')
    }

    const handleNewAttributeEnter = (event) => {
      if (event.which === 13) {
        addNewCustomAttribute(event.target.value)
      }
    }
    ...
    const changeBeat = (beatId) => {
      actions.changeBeat(cardId, beatId, ui.currentTimeline)
    }

    const changeLine = (lineId) => {
      actions.changeLine(cardId, lineId, ui.currentTimeline)
    }
      ...
      return addingAttribute ? (
        <div>
          ...
          <FormGroup>
            ...
            <input type="checkbox" onChange={handleNewAttributeTypeChange} />
          </FormGroup>
          <ButtonToolbar>
            <Button
              bsStyle="success"
              onClick={() => {
                if (newAttributeInputRef.current) {
                  addNewCustomAttribute(newAttributeInputRef.current.value)
                }
              }}
            >
              {t('Create')}
            </Button>
            <Button onClick={closeNewAttributeSection}>Cancel</Button>
          </ButtonToolbar>
        </div>
      ) : (
        <Button bsSize="small" onClick={onAddAttributeClicked}>

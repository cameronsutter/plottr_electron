  ...
  switch (action.type) {
    ...
    case ATTACH_BOOK_TO_CHARACTER: {
      ...
      const existingBookAttribute = Object.values(availableAttributes).find((attribute) => {
        return attribute.type === 'base-attribute' && attribute.name === 'bookIds'
      })
    ...
    case REMOVE_BOOK_FROM_CHARACTER: {
      ...
      // This shouldn't happen.  We're trying to remove a book from
      // the bookids attribute but it doesn't exist in the attributes
      // collection.
      if (!attributeId) {
        return state
      }

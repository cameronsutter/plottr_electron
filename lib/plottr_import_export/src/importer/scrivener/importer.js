...
export function generatePlaces(currentState, json, bookId, files, isNewFile) {
  json.forEach((item, key) => {
    ...
    if (matchFiles && matchFiles.length) {
      ...
      const content = objectifyRTFContents(
        currentState,
...
export function generateCharacters(currentState, json, bookId, files, isNewFile) {
  json.forEach((item, key) => {
    ...
    if (matchFile && matchFile.length) {
      ...
      if (RTFmatch) {
        const content = objectifyRTFContents(currentState, RTFmatch, 'character', name)
...
export function strippedDefaultAttributes(currentState, section, bookId, key, value) {
  ...
  if (section == 'characters') {
    const characterAttributeId = generateAttributes(currentState.attributes.characters, key, bookId)
...
export function generateAttributes(currentAtttributes, name, bookId) {
  const strippedAttribute = name.toLowerCase().trim()

  // Characters
  const attributeExist = currentAtttributes.find(
    (attr) => attr.name.toLowerCase().trim() == strippedAttribute
  )
  if (!attributeExist) {
    const nextAttributeId = nextId(currentAtttributes)
    const attributeType = isNotBaseAttribute(name) ? 'text' : 'base-attribute'
    ...
    currentAtttributes.forEach((attr, idx) => {
      if (attr.name.toLowerCase().trim() == strippedAttribute) {
        currentAtttributes[idx] = { ...attributeExist, ...values }
      }
    })
    return attributeExist.id
  }
}
...
export function objectifyRTFContents(currentState, fileContents, section, name) {
  try {
    ...
    return mergePairAttributes(currentState, flatAttributes)
  } catch (error) {
    return new Error(error)
  }
}
...
// Array [any] -> [Object]
export function mergePairAttributes(currentState, attributesList) {

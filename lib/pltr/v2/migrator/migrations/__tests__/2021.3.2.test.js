    ...
    describe('without the hierarchyLevels attribute', () => {
      ...
      describe('with the flat array representation of beats for three books', () => {
        ...
        it('should make a single top level beat per book', () => {
          expect(
            countBy(Object.values(migrated.beats.series.heap).filter((x) => x === null)).null
          ).toEqual(1)
          expect(
            countBy(Object.values(migrated.beats[1].heap).filter((x) => x === null)).null
          ).toEqual(1)
          expect(
            countBy(Object.values(migrated.beats[2].heap).filter((x) => x === null)).null
          ).toEqual(1)
        })
        it('should make all the beats children of the single top level beat', () => {
          expect(countBy(Object.values(migrated.beats.series.heap), (x) => x === 42).true).toEqual(
            1
          )
          expect(countBy(Object.values(migrated.beats[1].heap), (x) => x === 43).true).toEqual(2)
          expect(countBy(Object.values(migrated.beats[2].heap), (x) => x === 44).true).toEqual(2)
          expect(migrated.beats.series.children[42]).toEqual(expect.arrayContaining([10]))
          expect(migrated.beats[1].children[43]).toEqual(expect.arrayContaining([6, 7]))
          expect(migrated.beats[2].children[44]).toEqual(expect.arrayContaining([40, 41]))
        })

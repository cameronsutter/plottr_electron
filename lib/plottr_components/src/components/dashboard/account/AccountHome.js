...
const AccountHomeConnector = (connector) => {
  const {
    platform: {
      license: { useLicenseInfo },
      mpq,
    },
  } = connector
  checkDependencies({ useLicenseInfo, mpq })
  ...
  const AccountHome = ({ darkMode, hasCurrentProLicense, settings, started }) => {
    const [_licenseInfo, licenseInfoSize] = useLicenseInfo()
    const isFirstTime = () => !licenseInfoSize && !started && !hasCurrentProLicense

...
const BookListConnector = (connector) => {
  ...
  class BookList extends Component {
    ...
    addBookFromTemplate = (template) => {
      const { books, lines, cards, actions, lineActions, nextLineId, nextBeatId } = this.props
      ...
      const newLines = template.lines.map((l, index) => {
        ...
        l.id = newId
        // add them to the new book
        l.bookId = newBookId
        return l
      })
      // beats
      // TODO: this won't work with act structure on
      const newBeats = Object.keys(template.beats['1']['index']).map((id, index) => {
        const beat = template.beats['1']['index'][id]
        // give them a new id
        const newId = nextBeatId + index
        beatIdMap[beat.id] = newId // save old id -> new id
        beat.id = newId
        // add them to the new book
        beat.bookId = newBookId
        return beat
      })
      // cards
      ...
      lineActions.addLinesFromTemplate(
        [...cards, ...newCards],
        [...lines, ...newLines],
        newBeats,
        template,

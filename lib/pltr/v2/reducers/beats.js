...
export default function beats(state = INITIAL_STATE, action) {
  ...
  switch (action.type) {
    ...
    case INSERT_BEAT: {
      ...
      // If we don't get a parent id then make this a root node
      const parentId = nodeParent(state[actionBookId], action.siblingToLeft)
      const node = {
        autoOutlineSort: true,
        bookId: actionBookId,
        fromTemplateId: null,
        ...
        position: findNode(state[actionBookId], action.siblingToLeft).position + 1,
        time: 0,
        title: 'auto',
      }
      ...
      return {
        ...state,
        [actionBookId]: moveNextToSibling(newState, node.id, action.siblingToLeft),
      }
    }

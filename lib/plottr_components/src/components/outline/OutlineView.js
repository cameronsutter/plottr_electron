...
const OutlineViewConnector = (connector) => {
  ...
  const OutlineView = ({
    ...
    useEffect(() => {
      if (beatsRef.current) beatsRef.current.onscroll = scrollHandler

      setTimeout(() => {
        setMounted(true)
        if (outlineScrollPosition.timelineScrollPosition == null) return
        if (beatsRef.current) {
          scrollableRef.current.scrollTo(
            outlineScrollPosition.timelineScrollPosition.x,
            outlineScrollPosition.timelineScrollPosition.y,
            true
          )
        }
      }, 10)
      ...
      return () => {
        setMounted(false)
        if (beatsRef.current) {
          beatsRef.current.onScroll = null
    ...
    console.log('outline beatsRef.current', beatsRef.current)
    console.log('outlineScrollPosition', outlineScrollPosition)

    const scrollHandler = (e) => {
      console.log('outline scrollHandler', e.currentTarget)
      const position = {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        actions.recordOutlineScrollPosition(position)
      }, 500)
    }
    ...
    const renderBody = () => {
      ...
      return (
        <div className="outline__container tab-body">
          <Grid
            fluid
            className="outline__grid"
            wrapperRef={(ref) => {
              beatsRef.current = ref
            }}
          >
            <Row>

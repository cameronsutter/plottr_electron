...
const knownFilesPath = process.env.NODE_ENV == 'dev' ? 'known_files_dev' : 'known_files'

const knownFilesStore = new Store({name: knownFilesPath})

function addToKnown (filePath) {
  const files = knownFilesStore.store
  const alreadyExists = Object.entries(files).some(entry => path.normalize(entry[1].path) == path.normalize(filePath))
  if (alreadyExists) {
    const fileEntry = Object.entries(files).find(entry => path.normalize(entry[1].path) == path.normalize(filePath))

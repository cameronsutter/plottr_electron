...
describe('tree', () => {
  describe('given the idPropName "id"', () => {
    ...
    describe('given a tree with no children', () => {
      ...
      describe('and a function which produces the same node', () => {
        it('mapping that function should produce the same tree', () => {
          expect(map(tree, (x) => x)).toEqual(tree)
        })
      })
      describe('and a function which produces empty nodes', () => {
        it('the sole node should be empty', () => {
          const newTree = map(tree, () => ({}))
          expect(findNode(newTree, 1)).toEqual({})
        })
      })
      describe('and a function which increases the id attribute of the single node', () => {
        it('the sole node should have an increased id', () => {
          const newTree = map(tree, ({ id }) => ({
            id: id + 1,
          }))
          expect(findNode(newTree, 1)).toEqual({
            id: 2,
    ...
    describe('given a tree with a deep hierarchy', () => {
      ...
      describe('and a function which sums the id prop', () => {
        describe('and a function which sets the id prop to 0', () => {
          it('the sum of all id props should be zero', () => {
            const newTree = map(tree, () => ({ id: 0 }))
            expect(reduce(newTree, (acc, { id }) => acc + id, 0)).toEqual(0)
          })
        })

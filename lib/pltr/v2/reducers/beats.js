...
export default function beats(state = INITIAL_STATE, action) {
  ...
  switch (action.type) {
    ...
    case SET_HIERARCHY_LEVELS: {
      ...
      const newMaximumDepth = hierarchyLevels.length - 1

      return Object.keys(state).reduce((newState, bookId) => {
        const currentTree = newState[bookId]
        const maximumDepth = maxDepth(currentTree)
        const oldTopLevelNodes = children(currentTree, null)
        if (newMaximumDepth === maximumDepth) {
          // Same depth.  Nothing changes.
          return newState
        } else if (newMaximumDepth > maximumDepth) {
          // Add a level.
          const node = {
            autoOutlineSort: true,
            bookId: bookId,

...
const BackupsHomeConnector = (connector) => {
  const {
    platform: { useBackupFolders },
  } = connector
  checkDependencies({ useBackupFolders })
  ...
  const BackupsHome = ({ userId, settings }) => {
    ...
    return (
      <div className="dashboard__backups">
        ...
        <div className="dashboard__backups__wrapper">
          <DashboardErrorBoundary darMode={settings.user?.dark}>{body}</DashboardErrorBoundary>
        </div>
      </div>
    )
  }

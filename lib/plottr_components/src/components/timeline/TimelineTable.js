...
const TimelineTableConnector = (connector) => {
  ...
  class TimelineTable extends Component {
    ...
    componentDidUpdate() {
      ...
      const { visible } = this.props.toastProps
    ...
    handleCloseToast = () => {
      this.props.actions.showToastMessage(false)
    }
    ...
    renderToastMessage = () => {
      ...
      return (
        <div
          ...
          role="alert"
        >
          {this.getToastMessage(toastProps.cardAction, toastProps.newBookId)}
    ...
    render() {
      const { darkMode, orientation, isSmall, toastProps } = this.props
        ...
        return (
          ...
          >
            <table className="table-header-rotated">
              ...
              {toastProps.visible ? this.renderToastMessage() : null}
            </table>
          </div>
        )
      } else {
    ...
    return connect(
      (state) => {
        return {
          ...
          toastProps: selectors.toastPropsSelector(state.present),
        }
      },
      (dispatch) => {
        return {

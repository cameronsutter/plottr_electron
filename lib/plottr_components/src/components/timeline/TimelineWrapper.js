...
const TimelineWrapperConnector = (connector) => {
  ...
  const TimelineWrapper = ({
    ...
    const [focussedBeat, setFocussedBeat] = useState(null)
    ...
    const beatContextMenu = (beatId, target) => {
      setFocussedBeat(beatId)
      setContextMenuAnchor(target)
    }

    const insertPeerBeat = () => {
      beatActions.insertBeat(timelineBundle.currentTimeline, focussedBeat)
    }

    const stageBeatForDeletion = () => {
      setBeatToDelete(focussedBeat)
    }
      ...
      const rightItems = [
        ...
        <NavItem key="right-2">
          <Dropdown id="moar-dropdown">
            ...
            <Dropdown.Menu>
              <MenuItem onSelect={insertPeerBeat}>{t('Save as Template')}</MenuItem>
              <MenuItem divider />
      ...
      return (
        <SubNav>
          <Nav bsStyle="pills">
            ...
            <NavItem>
              <DropdownButton
                id="select-timeline-view-sub-nav"
                className="view-selector__select-line"
                title={timelineView}
              >
                <MenuItem key={'default'} onSelect={() => actions.setTimelineView('default')}>
                  <div className="view-selector__timeline-view-selector">{t('Default')}</div>
                </MenuItem>
                <MenuItem
                ...
                >
                  <div className="view-selector__timeline-view-selector">{t('Tabbed')}</div>
                </MenuItem>
                <MenuItem key={'stacked'} onSelect={() => actions.setTimelineView('stacked')}>
                  <div className="view-selector__timeline-view-selector">{t('Stacked')}</div>
                </MenuItem>
              </DropdownButton>
            </NavItem>
            <NavItem>
    ...
    const renderBeatContextMenu = () => {
      ...
      const hierarchyLevelName = 'DUMMY NAME'
            ...
            return (
              <Popover id="tab-beat-menu" contentStyleOverride={{ padding: 0 }}>
                <ul className="tab-dropdown-menu">
                  <MenuItem onSelect={insertPeerBeat}>{t(`Insert ${hierarchyLevelName}`)}</MenuItem>

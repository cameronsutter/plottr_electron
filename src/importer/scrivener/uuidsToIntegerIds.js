...
export const uuidsToIntegerIds = (jsonFile) => {
  ...
  const integerId = (x, type) => {
    if (isNumber(x)) return Number.parseInt(x)
  ...
  const integerIdOrNull = (x, type) => {
    if (!x || x === null || x === 'null') return null

    return integerId(x, type)
  }

  const integerIdOrSeries = (x, type) => {
  ...
  const fixKey = (key, originalFile, destinationFile) => {
    switch (key) {
      case 'books': {
        return {
          ...
          books: Object.keys(originalFile.books).reduce((acc, nextBookId) => {
            if (nextBookId === 'allIds') {
              return {
                ...acc,
                allIds: originalFile.books.allIds,
              }
            }
                  ...
                  (indexAcc, nextBeatId) => {
                    return {
                      ...
                      [integerId(nextBeatId, 'beatIds')]: {
                        ...originalFile.beats[nextBookId].index[nextBeatId],
        ...
        return {
          ...
          lines: originalFile.lines.map((line) => {
            return {
              ...
              bookId: integerIdOrSeries(line.bookId, 'bookIds'),
      ...
      case 'tags': {
        return {
          ...destinationFile,
          tags: originalFile.tags.map((tag) => {
            return {
              ...tag,
              id: integerId(tag.id, 'tagIds'),
            }
          }),
        }
      }

    ...
    switch (action.type) {
      ...
      case ADD_LINES_FROM_TEMPLATE: {
        const newCards = action.templateData.cards.map((c) => {
          // give it the correct beatId
          if (action.ignoreBeats) {
            c.beatId = action.cardToBeatIdMap[c.id]
          } else {
            // TODO:
            // think through this some more
            // c.beatId = action.nextBeatId + c.beatId
          }
          c.lineId = action.nextLineId + c.lineId // give it the correct lineId
          c.id = c.id + action.nextCardId // give it a new id
          c.fromTemplateId = action.templateData.id
          return c
        })
      ...
      case ADD_BOOK_FROM_TEMPLATE: {
        const newCards = action.templateData.cards.map((c) => {
          c.id = c.id + action.nextCardId // give it a new id
          c.lineId = action.nextLineId + c.lineId // give it the correct lineId
          c.beatId = action.nextBeatId + c.beatId // give it the correct beatId
          c.fromTemplateId = action.templateData.id
          return c
        })

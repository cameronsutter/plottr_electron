...
export const ensureStartsWithProtocol = (url) => {
  if (!url && url !== '') {
    return url
  }
  ...
  function iter(nodes) {
    ...
    return nodes.flatMap((n) => {
      ...
      switch (n.type) {
        ...
        case 'bulleted-list':
          if (helpers.card.richContentIsNonEmpty(n.children)) {
            return (n.children || []).flatMap((li) => {
              const bulletLeaf = (li, depth) => {
                if (li.type === 'bulleted-list') {
                  return li.children.flatMap((innerLi) => bulletLeaf(innerLi, depth + 1))
                }
        ...
        case 'link':
          return new ExternalHyperlink({
            ...
            link: ensureStartsWithProtocol(n.url),
          })
        case 'image-link':
          return new Paragraph({
            children: [

...
test('Specifically, good props check undefined (#1)', function (assert) {
  assert.plan(3)
  assert.is(checkPropTypes({}, {}, 'prop'), undefined)
  assert.is(checkPropTypes({ x: PropTypes.number }, { x: 1 }, 'prop'), undefined)
  assert.isNot(checkPropTypes({}, {}, 'prop'), null)
})
...
test('Bad props return a message', function (assert) {
  ...
  assert.equal(
    message,
    'Failed prop type: Invalid prop `x` of type `string` supplied to `C`, expected `number`.'
  )
})
...
test('Bad propTypes fail check', function (assert) {
  ...
  assert.equal(
    message,
    'C: prop type `x` is invalid; it must be a function, usually from React.PropTypes.'
  )

  message = checkPropTypes(
    {
      x: function () {
        return 1
      },
  ...
  assert.equal(
    message,
    'C: type specification of prop `x` is invalid; the type checker function must return `null` or an `Error` but returned a number. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).'
  )
})

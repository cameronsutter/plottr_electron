...
const TimelineTableConnector = (connector) => {
  ...
  class TimelineTable extends Component {
    ...
    setLength = () => {
      ...
      let newHeight = table.scrollHeight
      if (ui.orientation != 'horizontal') {
        newLength = table.scrollHeight
        newHeight = table.scrollWidth
      }
      if (this.state.tableLength != newLength) {
        this.setState({ tableLength: newLength, tableHeight: newHeight })
      }
    }
        ...
        return (
          ...
          >
            <table className="table-header-rotated">
              <TopRow tableHeight={this.state.tableHeight} />
              <tbody>{this.renderRows()}</tbody>
            </table>
          </div>
        )
      } else {
        return [<TopRow key="top-row" tableHeight={this.state.tableHeight} />, this.renderRows()]
      }
    }
  }

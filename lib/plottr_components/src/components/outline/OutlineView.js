...
const OutlineViewConnector = (connector) => {
  ...
  const OutlineView = ({
    ...
    const [outlineYScrollPosition, setOutlineYScrollPosition] = useState(0)
    ...
    useEffect(() => {
      if (outlineScrollPosition.y && scrollableRef.current) {
        setTimeout(() => {
          scrollableRef.current.scrollTo(0, outlineScrollPosition.y, true)
        }, 500)
      }
      document.addEventListener('scroll', (event) => {
        if (beatsRef.current && !isEqual(outlineScrollPosition.y, outlineYScrollPosition)) {
          setOutlineYScrollPosition(beatsRef.current.scrollTop)
        }
      })
    }, [])

    const handleScroll = (e) => {
      setOutlineYScrollPosition(e.target.scrollTop)
    }

    useEffect(() => {
      if (outlineYScrollPosition) {
        actions.recordOutlineScrollPosition({ x: 0, y: outlineYScrollPosition })
      }
    }, [outlineYScrollPosition])
          ...
          <Grid fluid className="outline__grid">
            <Row>
              ...
              <div
                ...
                sm={8}
                md={9}
                className="outline__grid__beats"
                ref={beatsRef}

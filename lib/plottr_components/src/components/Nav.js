...
class Nav extends React.Component {
  ...
  getNextActiveChild(offset, tabContainer) {
    const { children } = this.props
    const validChildren = children.filter(
      (child) => child.props.eventKey != null && !child.props.disabled
    )
    const { activeKey, activeHref } = this.getActiveProps(tabContainer)
  ...
  getTabProps(child, tabContainer, navRole, active, onSelect) {
    ...
    let { id, 'aria-controls': controls, eventKey, role, onKeyDown, tabIndex } = child.props
    ...
    if (navRole === 'tablist') {
      ...
      onKeyDown = createChainedFunction(
        (event) => this.handleTabKeyDown(onSelect, event, tabContainer),
        onKeyDown
      )
  ...
  handleTabKeyDown(onSelect, event, tabContainer) {
    let nextActiveChild

    switch (event.keyCode) {
      case keycode.codes.left:
      case keycode.codes.up:
        nextActiveChild = this.getNextActiveChild(-1, tabContainer)
        break

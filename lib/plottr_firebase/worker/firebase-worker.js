...
export const firebaseWorker = (logger, mintSessionClientId) => {
  ...
  const editFileName = (userId, fileId, newName) => {
    return sendPromise(EDIT_FILE_NAME, { userId, fileId, newName })
  }

  const updateAuthFileName = (userId, fileId, newName) => {
    return sendPromise(UPDATE_AUTH_FILE_NAME, { userId, fileId, newName })
  }
  ...
  try {
    worker.postMessage({
      ...
      messageId: uuidv4(),
      payload: { clientId: mintSessionClientId() },
    })
  } catch (error) {

...
function generateCustomAttribute(fileContents, section, name) {
  ...
  // WIP merge notes strings
  const output = flatAttributes.reduce((acc, item, index) => {
    if (item.text && item.text.length < 40) return acc
    else if (
      flatAttributes[index + 1] &&
      flatAttributes[index + 1].text &&
      flatAttributes[index + 1].text.length > 40 &&
      item.text &&
      item.text.length > 40
    ) {
      const merged = `${item.text} \n ${flatAttributes[index + 1].text}`
      const curr = item
      curr.text += merged
      acc.push(curr)
    }
  ...
  // find where to concat long paragraph (40 chars)
  return output
    .map((item, index) => ({
      [item.text]: output[index + 1] ? output[index + 1].text : '',
    }))

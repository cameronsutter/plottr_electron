...
import { FaExpandAlt, FaCompressAlt, FaCircle } from 'react-icons/fa'
  ...
  class BeatTitleCell extends PureComponent {
    ...
    render() {
      ...
      const innerKlass = cx(orientedClassName('beat__body', orientation), {
        ...
        hover: hovering && !this.props.showInsertGuide,
        dropping: inDropZone,
      })
      ...
      if (isSmall) {
        ...
        return (
          <Cell className="beat-heading__cell">
            <div
              className={beatKlass}
              ref={(ref) => {
                this.cellRef = ref

...
const attachBaseAttribute = (attributeName, value, action, state) => {
  return state.map((character) => {
    if (character.id === action.id) {
      ...
      return {
        ...character,
        ...(hasAttribute ? { [attributeName]: uniq([...character[attributeName], value]) } : {}),
        attributes,
      }
    }
    return character
      ...
      case DELETE_TAG: {
        return state.map((character) => {
          ...
          const characterHasAttribute = character.attributes.some((attribute) => {
            return action.attributeIdsForAllBooks.indexOf(attribute.id) > -1
          })
          if (characterHasAttribute) {
            return {
              ...character,
              attributes: character.attributes.map((attribute) => {
                if (action.attributeIdsForAllBooks.indexOf(attribute.id) > -1) {
                  return {
                    ...attribute,
        ...
        return state.map((character) => {
          ...
          if (characterHasAttribute) {
            return {
              ...
              attributes: character.attributes.map((attribute) => {
                if (
                  action.attributeIdsForAllBooks.indexOf(attribute.id) > -1 &&
                  action.category.id === attribute.value
                ) {
                  return {
                    ...attribute,
                    value: null,
      ...
      case EDIT_CHARACTER_ATTRIBUTE_VALUE: {
        return state.map((character) => {
          if (character.id === action.characterId) {
            const attributes = character.attributes || []
            const editingSeriesAttributeForBook = action.bookId !== 'all'
            const isAttributeToEdit = (attribute) => {
              return (
                (!editingSeriesAttributeForBook &&
                  attribute.id === action.attributeId &&
                  !attribute.bookId) ||
                (editingSeriesAttributeForBook &&
                  attribute.id === action.attributeId &&
                  attribute.bookId === action.bookId)
              )
            }

...
function hasNoValue(item, bookId, id) {
  const attributes = item.attributes || {}
  const attributeValues = attributes[bookId] || []
...
const noEntityHasAttributeBound = (entities, bookId, attrs) => {
  return attrs.reduce((acc, attr) => {
    if (attr.type == 'text') {
      acc.push(attr.name)
      return acc
    ...
    const changeable = entities.every((ch) => hasNoValue(ch, bookId, attr.id))
    if (changeable) acc.push(attr.name)
    return acc
  }, [])
}
...
export const characterCustomAttributesThatCanChangeSelector = createSelector(
  ...
  (characters, legacyAttributes, attributes, bookId) => {
    const characterBookAttributes =
      (attributes.characters &&
        attributes.characters.books &&
        attributes.characters.books[bookId]) ||
      []
    return [
      ...noEntityHasLegacyAttributeBound(characters, legacyAttributes),
      ...noEntityHasAttributeBound(characters, bookId, characterBookAttributes),
    ]
  }
)

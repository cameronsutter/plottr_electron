...
const confirmModalStyles = {
  content: { width: '50%', marginLeft: '25%', maxHeight: 400, borderRadius: 20 },
}
        ...
        return (
          <div onClick={stopPropogation} className="template-picker__confirmation__dialog-wrapper">
            <div className="template-picker__confirmation__header">
              <h3>{t('Configure this template')}</h3>
              <hr />
            </div>
            <div className="template-picker__confirmation__body">
              <p>
                {t(
                  'To which level in your book would you like to apply the {name} template? (Please select one)',
                  {
              ...
              <div className="template-picker__confirmation__row">
                {hierarchyLevels.map((level, index) => {
                  const selected = index == stagedPlotlineTemplate.level
                  const maxLevels = hierarchyLevels.length
                  console.log(index, stagedPlotlineTemplate, selected)
                  return (
                    <div
                      className={cx('template-picker__confirmation__option', { selected })}
                      key={`${level.name}-${index}`}
                      onClick={() => setSelectedDestination(index)}
                    >
                      <p>{t('Level {number}', { number: maxLevels - index })}</p>
                      <h3>{level.name}</h3>
                    </div>
                  )
                })}

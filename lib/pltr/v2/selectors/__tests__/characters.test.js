...
describe('characterBookCategoriesSelector', () => {
  ...
  describe('given a store with a character', () => {
    describe('and a single book', () => {
      it('should produce an array with a single category called "Characters Not In Book" with the book id in an array as the value', () => {
        const store = initialStore()
        store.dispatch(addCharacter('John Doe'))
        const initialState = store.getState().present
        expect(characterBookCategoriesSelector(initialState)).toEqual([
          {
            key: 'Characters Not In Book',
            'Characters Not In Book': [1],
          },
        ])
      })
    ...
    describe('and two books', () => {
      it('should produce an array with a single category, called "Characters Not In Book" with the book ids in the array as the value', () => {
        const store = initialStore()
        store.dispatch(addCharacter('John Doe'))
      ...
      describe('and that character is associated with the first book', () => {
        it('should produce an array with two categories, called "Characters In Book" & "Characters Not In Book" with the corresponding book ids', () => {
          const store = initialStore()
          store.dispatch(addCharacter('John Doe'))
          store.dispatch(addBook())
          store.dispatch(addBookToCharacter(1, 1))

...
const ChooseConnector = (connector) => {
  ...
  const Choose = ({ cancel, finalize, projects, templates }) => {
    ...
    const makeChoices = () => {
      const finalizedProjects = projects.filter((proj) => selectedProjects.includes(proj.fileURL))
    ...
    const allProjects = () => {
      setSelectedProjects(projects.map((p) => p.fileURL))
    }
    ...
    const ListProjects = () => {
      return projects
        ...
        .map((proj) => {
          const name = basename(proj.fileURL)
          const isChecked = selectedProjects.includes(proj.fileURL)
          return (
            <ListGroupItem
              key={proj.fileURL}
              onClick={() => toggleProject(!isChecked, proj.fileURL)}
            >
              <Checkbox
                inline
                checked={isChecked}
                onChange={(val) => toggleProject(val, proj.fileURL)}
              >
                <span>{name}</span>
              </Checkbox>
              <p className="secondary-text">{helpers.file.withoutProtocol(proj.fileURL)}</p>
            </ListGroupItem>
          )
        })
    }

...
const AccountConnector = (connector) => {
  ...
  const Account = ({
    ...
    const hideProButton = os == 'unknown' || hasCurrentProLicense || isInTrialMode
    ...
    const AllAccountInfo = () => {
      if (isInTrialMode) return <TrialInfo darkMode={darkMode} />
      if (isInTrialModeWithExpiredTrial) return <ExpiredView darkMode={darkMode} />

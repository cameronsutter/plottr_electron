...
describe('timelineReducer', () => {
  describe('given a timeline state with an empty hierarchy', () => {
    describe('and the add action', () => {
      const action = { type: ADD_HIERARCHY_LEVEL, bookId: 1, beatId: 3 }
      it('should create a root hierarchy level', () => {
        const emptyHierarchyState = {
          1: {
            id: 1,
            type: 'series',
            bookId: 'series',
        ...
        const newTimelineState = timelineReducer(emptyHierarchyState, action)
        expect(newTimelineState['1'].hierarchy).toEqual(
    ...
    describe('and the delete action type', () => {
      describe('and the book id 1', () => {
        describe('and the hierarchy level of 0', () => {
          const action = { type: DELETE_HIERARCHY_LEVEL, bookId: 1, level: 0, beatIds: [1] }
        ...
        describe('and the hierarchy level of 1', () => {
          ...
          it('should leave the hierarchy unchanged', () => {
            const newTimelineState = timelineReducer(undefined, action)
            expect(newTimelineState).toEqual(newFileTimeline)
          })
    ...
    describe('and the add action type', () => {
      describe('and the book id 1', () => {
        it('should enclose the level 1 of the hierarchy in the same parent and leave others alone', () => {
          ...
          const timelineForBook1 = newTimelineState['1']
          const timelineForBook2 = newTimelineState['2']
          const timelineForBookSeries = newTimelineState.series
          // Leaves the others alone
          expect(timelineForBook2).toEqual(threeLevelHierarchyState['2'])
          expect(timelineForBookSeries).toEqual(threeLevelHierarchyState.series)
      ...
      describe('and the book id 2', () => {
        it('should enclose the level 1 of the hierarchy in the same parent', () => {
          ...
          const timelineForBookSeries = newTimelineState.series
          // Leaves the others alone
          expect(timelineForBook1).toEqual(threeLevelHierarchyState['1'])
          expect(timelineForBookSeries).toEqual(threeLevelHierarchyState.series)
          // Wraps the children of 1
      ...
      describe('with the book id 1', () => {
        describe('and the level id 0', () => {
          it('should remove all of the children which have level 0 in book 1s hierarchy', () => {
            ...
            expect(timelineForBook2).toEqual(threeLevelHierarchyState['2'])
            expect(timelineForBookSeries).toEqual(threeLevelHierarchyState.series)
            // Deletes the leaves of 1
            expect(timelineForBook1.hierarchy[0].children[0]).toEqual([])
            expect(timelineForBook1.hierarchy[0].children[1]).toEqual([])
          })
        })
        describe('and the level id 1', () => {
          it('should remove all of the children which have level 1 and 0 in book 1s hierarchy', () => {
            ...
            expect(timelineForBookSeries).toEqual(threeLevelHierarchyState.series)
            // Deletes the leaves of 1
            expect(timelineForBook1.hierarchy[0]).toEqual([])
          })
        })
      })
    })
    describe('and the assign action type', () => {
      describe('with the book id 1', () => {
        describe('and the parent beat id 8', () => {
          it('should add the beat as a child of beat 8 in book 1s hierarchy', () => {
            const action = {
            ...
            expect(timelineForBook2).toEqual(threeLevelHierarchyState['2'])
            expect(timelineForBookSeries).toEqual(threeLevelHierarchyState.series)
            // Adds node to node with beat id 8
            expect(timelineForBook1.hierarchy[0]).toEqual([
              ...threeLevelHierarchyState[0].hierarchy[0],
              { level: 1, beatId: 12, children: [] },
            ])
          })
        })
      })
      describe('with the book id 2', () => {
        describe('and the parent beat id 8', () => {
          it('should do nothing because beat 8 is not in book 2s hierarchy', () => {
            ...
            const timelineForBook2 = newTimelineState['2']
            const timelineForBookSeries = newTimelineState.series
            // Leave everything the others alone
            expect(timelineForBook2).toEqual(threeLevelHierarchyState['2'])
            expect(timelineForBookSeries).toEqual(threeLevelHierarchyState.series)
            expect(timelineForBook1).toEqual(threeLevelHierarchyState['1'])
          })
        ...
        describe('and child beat id 4', () => {
          describe('and the parent beat id 7', () => {
            it('should move the beat from book 1 into book 2 as a child of beat 7', () => {
              ...
              const timelineForBookSeries = newTimelineState.series
              // Leave everything the others alone
              expect(timelineForBook1).toEqual({
                ...threeLevelHierarchyState['1'],
                hierarchy: [
                  {
                    ...threeLevelHierarchyState['1'].hierarchy[0],
                    children: [threeLevelHierarchyState['1'].hierarchy[0].children[1]],
                  },
                ],
              })
              expect(timelineForBookSeries).toEqual(threeLevelHierarchyState[1])
              expect(timelineForBook2).toEqual({
                ...threeLevelHierarchyState['2'],
                hierarchy: [
                  {
                    ...threeLevelHierarchyState['2'].hierarchy[0],
                    children: [
                      threeLevelHierarchyState['2'].hierarchy[0].children[0],
                      {
                        ...threeLevelHierarchyState['2'].hierarchy[0].children[1],
                        children: [
                          {
                            level: 1,
                            beatId: 4,

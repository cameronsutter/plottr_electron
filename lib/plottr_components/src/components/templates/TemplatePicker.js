...
const modalStyles = {
  content: { width: '50%', marginLeft: '25%', minHeight: 200, borderRadius: 20 },
}
const confirmModalStyles = {
  content: { width: '50%', marginLeft: '25%', maxHeight: 380, borderRadius: 20 },
}
...
const TemplatePickerConnector = (connector) => {
  ...
  const TemplatePicker = ({
    ...
    const [stagedPlotlineTemplate, setStagedPlotlineTemplate] = useState(null)
    ...
    useEffect(() => {
      ...
      if (isEqual(templates, newTemplates) && isEqual(customTemplates, newCustomTemplates)) return

      setTemplates(newTemplates)
      setCustomTemplates(newCustomTemplates)
    }, [setTemplates, setCustomTemplates, templates, customTemplates, userCustomTemplates])
    ...
    const saveTemplateEdit = (data) => {
      ...
      const newCustomTemplates = types.flatMap((type) =>
        userCustomTemplates.filter((template) => {
    ...
    const renderTemplateList = (list, category, type) => {
      ...
      return list
        .filter((template) => {
          const templateWasAlreadySelected =
            templatesAlreadySelected &&
            templatesAlreadySelected.some(({ id }) => {
              return id === template.id
            })
          return !templateWasAlreadySelected
        })
        .map((template) => {
      ...
      return listTypes.map((type) => {
        if (type == 'plotlines') {
          if (subCategory && subCategory !== 'all') {
            ...
            // all
            let list = templates[type].filter((template) => {
              const templateWasAlreadySelected =
                templatesAlreadySelected &&
                templatesAlreadySelected.some(({ id }) => {
                  return id === template.id
    ...
    const applyStagedTemplate = () => {
      onChooseTemplate(stagedPlotlineTemplate.template, selectedDestination)
      setStagedPlotlineTemplate(null)
    }

    const renderBody = () => {
      if (stagedPlotlineTemplate) {
        const levelToApplyTo = hierarchyLevels[stagedPlotlineTemplate.level]
        const selectedLevel = hierarchyLevels[selectedDestination] || levelToApplyTo
        return (
          <div onClick={stopPropogation} className="template-picker__confirmation__dialog-wrapper">
            <div className="template-picker__confirmation__header">
              <h3>{t('Where would you like to place this template?')}</h3>
              <hr />
            </div>
            <div className="template-picker__confirmation__body">
              <p>
                {t('{name} has a different act structure than your book.', {
                  name: stagedPlotlineTemplate.template?.name,
                })}{' '}
                {t.rich(
                  'It was configured to be placed within your <a>{level}s</a>, but you can change this.',
                  {
                    level: levelToApplyTo.name,
                    a: ({ children }) => <strong>{children}</strong>,
                  }
                )}
              </p>
              <div className="template-picker__confirmation__row">
                <FormGroup>
                  <FormControl
                    defaultValue={stagedPlotlineTemplate.level}
                    placeholder={levelToApplyTo.name}
                    componentClass="select"
                    onChange={withEventTargetValue(setSelectedDestination)}
                  >
                    {hierarchyLevels.map((level, index) => {
                      return (
                        <option
                          ...
                          value={index}
                          selected={index === stagedPlotlineTemplate.index}
                        >
                          {level.name}
                        </option>
                      )
                ...
                <p>
                  {t.rich('(Create new <a>{level}s</a> as needed)', {
              ...
              <p>
                <small>{t("Tip: If you don't like the result, you can undo and try again.")}</small>
              </p>
            </div>
      ...
      return (
        <div onClick={stopPropogation} className="template-picker__dialog-wrapper">
          {renderDelete()}
    ...
    if (modal) {
      return (
        <PlottrModal
          ...
          style={stagedPlotlineTemplate ? confirmModalStyles : modalStyles}
        >
          {renderBody()}
        </PlottrModal>
      )
  ...
  TemplatePicker.propTypes = {
    ...
    starterTemplates: PropTypes.array.isRequired,
    userCustomTemplates: PropTypes.array.isRequired,
    templatesAlreadySelected: PropTypes.array,

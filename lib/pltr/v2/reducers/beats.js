...
import { addNode, nextId } from './tree'
  ...
  switch (action.type) {
    case ADD_BEAT: {
      ...
      const node = {
        ...
        bookId: actionBookId,
        fromTemplateId: null,
        id: nextId('id')(state[actionBookId]),
        position,
        time: 0,
        title: action.title,
      }
      return add(state[actionBookId], parentId, node)
    }
      ...
      return [
        {
          ...
          time: 0,
          title: action.title || 'auto',
        },
        ...state,
      ]

    case DELETE_HIERARCHY_LEVEL: {
      return state.filter(({ id }) => !action.beatIds.includes(id))
    }

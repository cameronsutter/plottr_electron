...
const combinedAttributesForCharacter = (
  ...
  const legacyAttributes = customAttributes
    .filter((customAttribute) => {
      const newAttributeExists = allAttributes.some((attribute) => {
        return attribute.name === customAttribute.name
      })
      return !newAttributeExists && typeof character[customAttribute.name] !== 'undefined'
    })
    .map((customAttribute) => {

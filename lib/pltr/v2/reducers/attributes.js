  ...
  (state = INITIAL_STATE, action) => {
    switch (action.type) {
      ...
      case EDIT_CHARACTER_ATTRIBUTE_METADATA: {
        const characterAttributeState = state.characters || EMPTY_ATTRIBUTE_STATE
        const characterAttributes = characterAttributeState.books[action.bookId] || []
        const hasAttribute = characterAttributes.some((attribute) => {
        ...
        const characterAllAttributes = characterAttributeState.books.all
        const allHasAttribute = characterAllAttributes.some((attribute) => {
          if (attribute.id === action.id) {
            return {
              ...attribute,
              name: action.name,
        ...
        if (!hasAttribute && !allHasAttribute) {
          return state
        }
        const bookId = hasAttribute ? action.bookId : 'all'
        const attributes = hasAttribute ? characterAttributes : characterAllAttributes
        ...
        return {
          ...
          characters: {
            ...
            books: {
              ...characterAttributeState.books,
              [bookId]: attributes.map((attribute) => {
                if (attribute.id === action.id) {
                  return {
                    ...attribute,
                    name: action.name,
      ...
      case DELETE_CHARACTER_ATTRIBUTE: {
        ...
        return {
          ...
          characters: {
            ...
            books: {
              ...characterAttributeState.books,
              [bookId]: attributes.filter((attribute) => {

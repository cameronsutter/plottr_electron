      ...
      case ATTACH_BOOK_TO_CHARACTER:
        return state.map((character) => {
          if (character.id === action.id) {
            ...
            const attributes = hasAttribute
              ? newAttributes.map((attribute) => {
                  if (attribute.id === attributeId) {
                    return {
                      ...attribute,
                      value: uniq([...attribute.value, action.bookId]),
                    }
                  }
                  return attribute
                })
              : [...newAttributes, { id: attributeId, value: [action.bookId] }]

            return {
              ...character,
              bookIds: uniq([...character.bookIds, action.bookId]),
              attributes,
            }
          }
          return character

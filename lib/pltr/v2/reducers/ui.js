...
export default function ui(state = defaultUI, action) {
  ...
  switch (action.type) {
    ...
    case SET_OUTLINE_FILTER:
      if (action.filter === null || !action.filter) filter = null
      ...
      else if (!Array.isArray(state.outlineFilter) || !state.outlineFilter) filter = [action.filter]
      else if (!!state.outlineFilter && Array.isArray(state.outlineFilter))

...
export const characterBookCategoriesSelector = createSelector(
  ...
  (bookId, characters) => {
    const categoryMembership = characters.reduce((acc, character) => {
      ...
      return {
        ...acc,
        'Characters Not In Book': [character.id, ...(acc['Characters Not In Book'] || [])],
      }
    }, {})
    return [
      categoryMembership['Characters In Book']?.length > 0
        ...
        : [],
      categoryMembership['Characters Not In Book']?.length > 0
        ? [
            {
              displayHeading: true,
              key: 'Characters Not In Book',
              'Characters Not In Book': categoryMembership['Characters Not In Book'],
              lineAbove: false,
            },
          ]
        : [],

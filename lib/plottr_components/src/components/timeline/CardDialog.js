...
const CardDialogConnector = (connector) => {
  ...
  const CardDialog = ({
    ...
    const [newLineId, setNewLineId] = useState(lineId)
    const [newBeatId, setNewBeatId] = useState(beatId)
    ...
    const getCurrentLine = () => {
      return lines.find((l) => l.id == newLineId)
    }
    ...
    const renderLeftSide = () => {
      ...
      const DropDownTitle = <BeatItemTitle beat={beats.find(({ id }) => newBeatId === id)} />

  ...
  const TemplatePicker = ({
    ...
    const renderBody = () => {
      if (stagedPlotlineTemplate) {
        const defaultLevel = hierarchyLevels[stagedPlotlineTemplate.level]
        const selectedLevel = hierarchyLevels[selectedDestination] || defaultLevel
        return (
            ...
            <div className="template-picker__confirmation__body">
              ...
              <div className="template-picker__confirmation__row">
                {hierarchyLevels.map((level, index) => {
                  ...
                  const isRecommended = index == defaultLevel.level
                  const maxLevels = hierarchyLevels.length
                  return (
                    <div
                      className={cx('template-picker__confirmation__option', {
                        selected,
                    ...
                    >
                      <p className="caps">{t('Level {number}', { number: maxLevels - index })}</p>
                      <h3>{level.name}</h3>
                      {isRecommended ? <p className="small">{t('(Recommended)')}</p> : null}
                    </div>
                  )
                })}
              </div>

  ...
  (state = initialState, action) => {
    ...
    switch (action.type) {
      ...
      case ADD_BOOK_FROM_TEMPLATE: {
        const linesInBook_ = state.filter(
          (l) => l.bookId == action.bookId || l.bookId == action.newBookId
        ).length
        const newLines = action.templateData.lines.map((l, index) => {
          l.id = l.id + action.nextLineId // give it a new id
          l.bookId = action.newBookId || action.bookId // add it to the new/current book
          l.fromTemplateId = action.templateData.id
          if (!l.color || l.color == nextColor(0)) {
            l.color = nextColor(linesInBook_ + index)
          }
          return l
        })

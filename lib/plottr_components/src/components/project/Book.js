...
const BookConnector = (connector) => {
  ...
  class Book extends Component {
    ...
    handleOpenBookDialog = () => {
      const { uiActions, book, bookNumber } = this.props
      uiActions.setBookDialogOpen(book.id, bookNumber)
    }
    ...
    handleClickAddBook = () => {
      const { uiActions, books } = this.props
      const nextBookNumber = books.allIds.length + 1
      uiActions.setBookDialogOpen(null, nextBookNumber)
    }

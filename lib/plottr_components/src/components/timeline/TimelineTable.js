...
const TimelineTableConnector = (connector) => {
  ...
  class TimelineTable extends Component {
    ...
    componentDidUpdate() {
      ...
      if (this.state.toastProps.visible) {
        setTimeout(() => {
    ...
    handleCloseToast = () => {
      this.setState({
        toastProps: { visible: false, action: null, cardId: null, bookId: null },
      })
    }

    handleShowToast = (action, cardId, bookId) => {
      this.setState({ toastProps: { visible: true, action, cardId, bookId } })
    }

    getToastMessage = () => {
      const { bookId, action, cardId } = this.state.toastProps
      if (action == 'move' && bookId) {
        const { books, actions } = this.props
        const bookTitle = this.bookTitle(books[bookId])
        return (
          <div className="toast-message-with-anchor">
            {t('Woohoo! Scene card moved to: ')}
            <a href="#" onClick={() => actions.changeCurrentTimeline(bookId)}>
              {` ${bookTitle}`}
            </a>
          </div>
        )
      } else if (action == 'move' && Array.isArray(cardId)) {
        return t('Woohoo! Scene cards moved')
      } else if (action == 'move' && !bookId) {
        return t('Woohoo! Scene card moved')
    ...
    render() {
      ...
      const { visible } = this.state.toastProps
        ...
        return (
          ...
          >
            <table className="table-header-rotated">
              ...
              {visible ? this.renderToastMessage() : null}
            </table>
          </div>
        )
      } else {

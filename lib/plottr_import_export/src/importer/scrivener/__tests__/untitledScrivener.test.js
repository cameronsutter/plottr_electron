import untitledScrixJSON from './fixtures/untitled-jsonified.json'
import { keys, camelCase, values } from 'lodash'
...
import { untitledRelevantFiles } from './fixtures/scrivener-untitled-relevantFiles'
...
let bookId = 1
const currentState = emptyFile(BOOK_TITLE)
currentState.beats = {
...
describe('UntitledScrivenerImporter', () => {
  const manuscript = untitledScrixJSON['ScrivenerProject']['Binder']['BinderItem'].find(
    (n) =>
      n['_attributes']['Type'] == 'DraftFolder' ||
      (n['Title'] && n['Title']['_text'] && n['Title']['_text'] == 'Manuscript')
  )

  const sectionsJSON = untitledScrixJSON['ScrivenerProject']['Binder']['BinderItem'].filter(
  ...
  describe('extractStoryLines', () => {
    const isNewFile = true
    const json = extractStoryLines(
      currentState,
      manuscript,
      BOOK_TITLE,
      bookId,
      isNewFile,
      untitledRelevantFiles
    )

    it('should return an Object', () => {
      expect(isAnObject(json)).toBeTruthy()
    })
  })
  ...
  describe('createNewCharacter', () => {
    ...
    it('should return an id interger', () => {
      expect(newCharacter1).toBeANaturalNumber()
      expect(newCharacter2).toBeANaturalNumber()
    })
  ...
  describe('createNewNote', () => {
    ...
    it('should return an id interger', () => {
      expect(newNote1).toBeANaturalNumber()
      expect(newNote2).toBeANaturalNumber()
    })
  ...
  describe('createNewCategory', () => {
    ...
    it('should return an id interger', () => {
      expect(newCategory1).toBeANaturalNumber()
      expect(newCategory2).toBeANaturalNumber()
    })
  ...
  describe('createNewTag', () => {
    ...
    it('should return an id interger', () => {
      expect(newTag1).toBeANaturalNumber()
      expect(newTag2).toBeANaturalNumber()
    })
  ...
  describe('getMatchedRelevantFiles', () => {
    const files = getMatchedRelevantFiles(untitledRelevantFiles, UUID)
  ...
  describe('getVer_2_7_matchName', () => {
    const file = untitledRelevantFiles.filter((f) => !UUIDFolderRegEx.test(f.filePath))
  ...
  describe('objectifyRTFContents', () => {
    ...
    it('should return an array of objects from the file or return an error about missing contents of the file', () => {
      charactersBinderItem.forEach((item, key) => {
        ...
        const matchFile = getMatchedRelevantFiles(untitledRelevantFiles, id)
  ...
  describe('isATag', () => {
    ...
    charactersBinderItem.forEach((item, key) => {
      const name =
        item['Title'] && item['Title']['_text'] ? item['Title']['_text'] : `Character ${key}`
      const matchFile = getMatchedRelevantFiles(untitledRelevantFiles, UUID)
      const RTFmatch = matchFile.find((item) => item.filePath.includes('.rtf'))
      const customAttributes = objectifyRTFContents(RTFmatch, sectionItem, name)
  ...
  describe('mergePairAttributes', () => {
    ...
    it('should return an array of objects', () => {
      ...
      mergedAttributes.forEach((attr) => {
        expect(isAnObject(attr)).toBeTruthy()
      })
    })
  })
  ...
  describe('isNotBaseAttribute', () => {
    ...
    it('should return false for values found in EXCLUDED_ATTRIBUTES', () => {
      testAttributes.forEach((attr) => {
        if (EXCLUDED_ATTRIBUTES.includes(camelCase(attr.toLowerCase().trim()))) {
          expect(isNotBaseAttribute(camelCase(attr))).toBeFalsy()
        } else {
          expect(isNotBaseAttribute(camelCase(attr))).toBeTruthy()
        }
      })
    })
  })
  ...
  describe('createAttribute', () => {
    const newAttributeId1 = createNewAttribute(
      currentState.attributes.characters,
      TEST_STRING_1,
      bookId
    )
    const newAttributeId2 = createNewAttribute(
      currentState.attributes.characters,
      TEST_STRING_2,
      bookId
    )
    const newAttributeId3 = createNewAttribute(
      currentState.attributes.characters,
      TEST_STRING_3,
      bookId
    )
    const newAttributeId4 = createNewAttribute(
      currentState.attributes.characters,
      TEST_STRING_3,
      bookId
    )
    ...
    it('should return an number', () => {
      expect(newAttributeId1).toBeANaturalNumber()
      expect(newAttributeId2).toBeANaturalNumber()
      expect(newAttributeId3).toBeANaturalNumber()
      expect(newAttributeId4).toBeANaturalNumber()
    })

    it('should return an incremented value by 1', () => {
      expect(newAttributeId2).toBeGreaterThan(newAttributeId1)
      expect(newAttributeId2 - newAttributeId1).toBe(1)
    })

    it('should return found attribute id if key of attribute exists', () => {
      expect(newAttributeId3).toEqual(newAttributeId4)
    })
  })

    ...
    return connect(
      (state, props) => {
        ...
        if (props.type == 'characters') {
          customAttributes = selectors.characterSortCAnamesSelector(state.present)
          attr = selectors.characterSortSelector(state.present)
          items = [t('name'), t('description'), ...customAttributes]
        } else if (props.type == 'places') {
          customAttributes = selectors.placeSortCAnamesSelector(state.present)
          attr = selectors.placeSortSelector(state.present)
          items = [t('name'), t('description'), ...customAttributes]
        } else {
          customAttributes = selectors.noteSortCAnamesSelector(state.present)
          attr = selectors.placeSortSelector(state.present)

...
const TimelineTableConnector = (connector) => {
  ...
  class TimelineTable extends Component {
    ...
    setLength = () => {
      const { tableRef, orientation, isSmall, isMedium, timelineViewIsStacked } = this.props
      if (isSmall) return
      ...
      if (orientation === 'horizontal') {
        ...
        if (row) {
          newLength = Array.from(row.children)
            .slice(1, timelineViewIsStacked ? undefined : -1) // The first table cell is note above the line
            .reduce((acc, nextNode) => {
    ...
    componentDidUpdate() {
      ...
      const { timelineViewIsStacked, timelineViewIsTabbed } = this.props

      if (timelineViewIsStacked || timelineViewIsTabbed) {
    ...
    renderVertical() {
      ...
      const renderedBeats = beats.map((beat, idx) => {
        ...
        const beatTitle = (
          <MousePositionContext.Provider value={this.state.mouseXY}>
            <BeatTitleCell beatId={beat.id} handleReorder={this.handleReorderBeats} />

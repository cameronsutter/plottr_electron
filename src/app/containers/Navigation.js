...
class Navigation extends Component {
    ...
    return (
      <ul className="nav navbar-nav navbar-right" style={{ marginRight: '15px' }}>
        <li>
          <a
            href="#"
            style={{ color: 'hsl(210, 83%, 53%)' }}
            onClick={() => ipcRenderer.send('open-buy-window')}
          >
        ...
        <li>
          <a
            href="#"
            style={{ color: 'hsl(210, 83%, 53%)' }}
            onClick={() => ipcRenderer.send('verify-from-expired')}
          >
            <FaKey /> {i18n('Activate License')}
          </a>
        </li>
      </ul>
    )
  ...
  render() {
    let klasses = cx('navbar', 'navbar-default', 'navbar-fixed-top', {
      'navbar-inverse': this.props.ui.darkMode,
    })
    return (
        ...
        <div className="collapse navbar-collapse">
          ...
          <ul className="nav navbar-nav">
            <li className={this.isActive('story')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('story')}>
                {i18n('Project')}
              </a>
            </li>
            <li className={this.isActive('timeline')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('timeline')}>
                {i18n('Timeline')}
              </a>
            </li>
            <li className={this.isActive('outline')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('outline')}>
                {i18n('Outline')}
              </a>
            </li>
            <li className={this.isActive('notes')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('notes')}>
                {i18n('Notes')}
              </a>
            </li>
            <li className={this.isActive('characters')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('characters')}>
                {i18n('Characters')}
              </a>
            </li>
            <li className={this.isActive('places')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('places')}>
                {i18n('Places')}
              </a>
            </li>
            <li className={this.isActive('tags')}>
              <a href="#" onClick={() => this.props.actions.changeCurrentView('tags')}>
                {i18n('Tags')}
              </a>
            </li>
            {this.state.isDev ? (
              <li className={this.isActive('analyzer')}>
                <a href="#" onClick={() => this.props.actions.changeCurrentView('analyzer')}>

...
const addLevelToHierarchy = (newParentId, currentTree, bookId) => {
  const node = {
    autoOutlineSort: true,
    bookId: bookId,
    fromTemplateId: null,
...
const adjustHierarchyLevels = (targetHierarchyDepth) => (nextParentId, currentTree, bookId) => {
  const maximumDepth = maxDepth(currentTree)
  if (targetHierarchyDepth === maximumDepth) {
    return currentTree
  } else if (targetHierarchyDepth > maximumDepth) {
    return addLevelToHierarchy(nextParentId, currentTree, bookId)
  } else {
...
export default function beats(state = INITIAL_STATE, action) {
  ...
  switch (action.type) {
    ...
    case SET_HIERARCHY_LEVELS: {
      ...
      const adjustHierarchy = adjustHierarchyLevels(nextId(state), targetHierarchyDepth)

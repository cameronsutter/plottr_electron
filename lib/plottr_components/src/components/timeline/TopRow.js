...
const TopRowConnector = (connector) => {
  ...
  class TopRow extends Component {
    ...
    renderPaddingCells(count) {
      const paddingCells = []
      for (let i = 0; i < count; ++i) {
        paddingCells.push(<Cell key={`padding-cell-${i}-1`} className="beat__heading-spacer" />)
        paddingCells.push(<Cell key={`padding-cell-${i}-2`} className="beat__heading-spacer" />)
      }
      return paddingCells
    }
  ...
  const {
    ...
    topTierBeatsInThreeTierArrangement,
    secondTierBeatsInAtLeastTwoTierArrangement,
    ...
    return connect(
      (state) => {
        ...
        return {
          ...
          topTierBeats: topTierBeatsInThreeTierArrangement(state.present),
          secondTierBeats: secondTierBeatsInAtLeastTwoTierArrangement(state.present),

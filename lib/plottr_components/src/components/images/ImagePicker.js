...
const ImagePickerConnector = (connector) => {
  ...
  class ImagePicker extends Component {
    ...
    deleteImage = () => {
      const { bookId, isBookImage, chooseImage, actions } = this.props

      if (this.state.deleting) {
        if (isBookImage && bookId) {
          actions.deleteImage(bookId, isBookImage)
        } else {
          actions.deleteImage(this.state.selectedId)
        }
        chooseImage(null)
  ...
  ImagePicker.propTypes = {
    ...
    bookId: PropTypes.PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    isBookImage: PropTypes.bool,
  }

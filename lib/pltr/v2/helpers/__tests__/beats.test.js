  ...
  describe('given a file with one level', () => {
    ...
    describe('and a template with two levels', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        const topLevelBeats = tree.children(two_level_biased_top_plotline_template.beats['1'], null)
        it('should have more cards', () => {
          expect(result.cards.length).toBeGreaterThanOrEqual(file_with_one_level.cards.length)
        })
        it('should have the cards it used to have', () => {
        ...
        it('should have at least as many beats as the top level of the template', () => {
          const templateNumberOfBeats = beatsLength(topLevelBeats)
          const resultNumberOfBeats = beatsLength(result.beats['1'])
          expect(resultNumberOfBeats).toBeGreaterThanOrEqual(templateNumberOfBeats)
        })
        it('should add the plotlines from the template', () => {
          expect(result.lines).toEqual(
            expect.arrayContaining(two_level_biased_top_plotline_template.lines)
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should insert cards on the plotlines from the template', () => {
          const preExistingLines = two_level_biased_top_plotline_template.lines.map((line) => {
            return line.id
          })
          const addedCards = result.cards.filter((card) => {
            return preExistingLines.indexOf(card.lineId) === -1
          ...
          const topLevelBeatIds = topLevelBeats.map(({ id }) => id)
          const templateCards = two_level_biased_top_plotline_template.cards.filter(
            ({ beatId }) => {
              return topLevelBeatIds.indexOf(beatId) !== -1
            }
          )
          expect(two_level_biased_top_plotline_template.cards.length).toEqual(addedCards.length)
      ...
      describe('and the template is biased to the middle', () => {
        ...
        const bottomLevelBeats = tree.children(
          two_level_biased_middle_plotline_template.beats['1'],
          null
        )
        it('should have more cards', () => {
          expect(result.cards.length).toBeGreaterThanOrEqual(file_with_one_level.cards.length)
        ...
        it('should have at least as many beats as the bottom level of the template', () => {
          const templateNumberOfBeats = beatsLength(bottomLevelBeats)
          const resultNumberOfBeats = beatsLength(result.beats['1'])
          expect(resultNumberOfBeats).toBeGreaterThanOrEqual(templateNumberOfBeats)
        })
        it('should add the plotlines from the template', () => {
          expect(result.lines).toEqual(
            expect.arrayContaining(two_level_biased_middle_plotline_template.lines)
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should insert cards on the plotlines from the template', () => {
          const preExistingLines = two_level_biased_middle_plotline_template.lines.map((line) => {
            return line.id
          })
          const addedCards = result.cards.filter((card) => {
            return preExistingLines.indexOf(card.lineId) === -1
          ...
          const bottomLevelBeatIds = bottomLevelBeats.map(({ id }) => id)
          const templateCards = two_level_biased_middle_plotline_template.cards.filter(
            ({ beatId }) => {
              return bottomLevelBeatIds.indexOf(beatId) !== -1
            }
          )
          expect(two_level_biased_middle_plotline_template.cards.length).toEqual(addedCards.length)
          expect(addedCards).toEqual(expect.arrayContaining(templateCards))
        })
      })
      describe('and the template is biased to the bottom', () => {
        ...
        const bottomLevelBeats = tree.children(
          two_level_biased_bottom_plotline_template.beats['1'],
          null
        )
        it('should have more cards', () => {
          expect(result.cards.length).toBeGreaterThanOrEqual(file_with_one_level.cards.length)
        ...
        it('should insert cards on the plotlines from the template', () => {
          const preExistingLines = two_level_biased_bottom_plotline_template.lines.map((line) => {
            return line.id
          })
          const addedCards = result.cards.filter((card) => {
            return preExistingLines.indexOf(card.lineId) === -1
          ...
          const bottomLevelBeatIds = bottomLevelBeats.map(({ id }) => id)
          const templateCards = two_level_biased_bottom_plotline_template.cards.filter(
            ({ beatId }) => {
              return bottomLevelBeatIds.indexOf(beatId) !== -1
            }
          )
          expect(two_level_biased_bottom_plotline_template.cards.length).toEqual(addedCards.length)
          expect(addedCards).toEqual(expect.arrayContaining(templateCards))
        })
      })
    })
    describe('and a template with three levels', () => {
      describe('and the template is biased to the top', () => {
        ...
        const topLevelBeats = tree.children(
          three_level_biased_top_plotline_template.beats['1'],
          null
        )
        it('should have more cards', () => {
          expect(result.cards.length).toBeGreaterThanOrEqual(file_with_one_level.cards.length)
      ...
      describe('and the template is biased to the middle', () => {
        ...
        const topLevelBeats = tree.children(
          three_level_biased_middle_plotline_template.beats['1'],
          null
        )
        ...
        it('should have at least as many beats as the middle level of the template', () => {
          const templateNumberOfBeats = beatsLength(middleLevelBeats)
          const resultNumberOfBeats = beatsLength(result.beats['1'])
          expect(resultNumberOfBeats).toBeGreaterThanOrEqual(templateNumberOfBeats)
        })
        it('should add the plotlines from the template', () => {

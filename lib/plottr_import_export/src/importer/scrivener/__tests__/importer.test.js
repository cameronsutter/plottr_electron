...
import { camelCase, cloneDeep, keys, values } from 'lodash'
...
const CHARLES_BINGLEY_CONCATENATED_DESCRIPTION =
  "Mr. Darcy's best friend \n" +
  '\n' +
  " A sweet, easygoing man with an open heart who moves to Netherfield at the book's beginning. \n" +
  '\n' +
  ' While she is tolerable, she is not handsome enough to tempt HIM! Gasp! \n' +
  '\n' +
...
describe('ScrivenerImporter', () => {
  ...
  describe('getManuscriptSectionItems', () => {
    ...
    const files = getMatchedRelevantFiles(relevantFiles, UUID)
    let fileContents = { txtContent: createSlateParagraph(''), rtfContents: { contents: [] } }
  ...
  describe('concatenateValues', () => {
    ...
    it('should concatenate text if 2 or more consecutive texts is not bold or is not an attribute title', () => {
      concatenated.forEach((item, idx) => {
        if (
          !!concatenated[idx - 1] &&
          concatenated[idx - 1].isBold &&
          camelCase(concatenated[idx - 1].text) == 'description' &&
          !!item.text &&
          item.text.includes(`Mr. Darcy's best friend`)
        ) {
          expect(item.text).toEqual(CHARLES_BINGLEY_CONCATENATED_DESCRIPTION)
        }
      })
    })
  })

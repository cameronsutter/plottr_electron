...
const ExportDialogConnector = (connector) => {
  ...
  function ExportDialog(props) {
    ...
    const beforeExport = () => {
      const { actions } = props
      if (options[type].outline.filter.length) {
        actions.overrideOutlineFilter(options[type].outline.filter)
      }
      doExport()
    }
          ...
          <div className="export-dialog__header">
            <div className="export-dialog__type-chooser">
              ...
              <div className="right-side">
                <Chooser />
                <Button bsStyle="success" disabled={!type} onClick={() => beforeExport()}>

...
import { chunk, flatten, cloneDeep } from 'lodash'
...
const BookListConnector = (connector) => {
  ...
  class BookList extends Component {
    ...
    addBook = (template) => {
      if (template) {
        return this.addBookFromTemplate(template)
      }
    ...
    addBookFromTemplate = (template) => {
      const { books, cards, actions, lineActions, beatActions, nextLineId, nextBeatId } = this.props
      ...
      actions.addBook()
      // lines
      const newLines = template.lines.map((l, index) => {
        // give it a new id
        const newId = nextLineId + index
        lineIdMap[l.id] = newId // save old id -> new id
        l.id = newId
        // add it to the new book
        l.bookId = newBookId
        l.fromTemplateId = template.id
        return l
      })
      // beats
      const beats = cloneDeep(template.beats['1'])
      Object.keys(beats['index']).forEach((oldId, index) => {
        const newId = nextBeatId + index
        const newIdStr = `${newId}`
        beatIdMap[oldId] = newId // save old id -> new id
        //index
        // give it a new id
        beats['index'][newId] = cloneDeep(beats['index'][oldId])
        delete beats['index'][oldId]
        beats['index'][newId].id = newId
        // add it to the new book
        beats['index'][newId].bookId = newBookId
        beats['index'][newId].fromTemplateId = template.id
        //heap
        if (beats['heap'][oldId] !== undefined) {
          beats['heap'][newIdStr] = cloneDeep(beats['heap'][oldId])
          delete beats['heap'][oldId]
        }
        Object.keys(beats['heap']).forEach((id) => {
          if (beats['heap'][id] == oldId) {
            beats['heap'][id] = newId
          }
        })
        //children
        if (beats['children'][oldId]) {
          beats['children'][newIdStr] = cloneDeep(beats['children'][oldId])
          delete beats['children'][oldId]
        }
        Object.keys(beats['children']).forEach((id) => {
          const foundIndex = beats['children'][id].indexOf(parseInt(oldId))
          if (foundIndex != -1) {
            beats['children'][id].splice(foundIndex, 1, newId)
          }
        })
      })
      // cards
      const newCards = template.cards.map((c) => {
        // change their line & beat ids
        c.lineId = lineIdMap[c.lineId]
        c.beatId = beatIdMap[c.beatId]
        c.fromTemplateId = template.id
        return c
      })

      lineActions.addLinesFromTemplate([...cards, ...newCards], newLines, [], template, newBookId)
      beatActions.addBeatsFromTemplate(newBookId, beats)
    }

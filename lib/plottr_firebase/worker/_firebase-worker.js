  ...
  const replyToPromise =
    ...
    (result) => {
      try {
        ...
        logger.error('Error posting message back', error.message, error)
        try {
          self.postMessage({
  ...
  switch (type) {
    case INITIALISE_WORKER: {
      ...
      try {
        ...
        logger.error('Error logging error to main process: ', error.message, error)
      }
      return
    }
    case EDIT_FILE_NAME: {
    ...
    case LISTEN_TO_FILES: {
      ...
      const unsubscribe = listenToFiles(userId, (result) => {
        try {
          ...
          logger.error('Error replying to LISTEN_TO_FILES: ', error.message, error)
        }
      })
      unsubscribeFunctions.set(messageId, unsubscribe)
      return
    ...
    case ON_SESSION_CHANGE: {
      const unsubscribe = onSessionChange((user) => {
        try {
          ...
          logger.error('Error replying to ON_SESSION_CHANGE: ', error.message, error)
        }
      })
      unsubscribeFunctions.set(messageId, unsubscribe)
      return
    ...
    case CURRENT_USER: {
      ...
      try {
        ...
        logger.error('Error replying to CURRENT_USER: ', error.message, error)
      }
      return
    }
    case PATCH: {
    ...
    case GET_ID_TOKEN_RESULT: {
      ...
      if (user) {
        ...
        try {
          ...
          logger.error('Error replying to GET_ID_TOKEN_RESULT: ', error.message, error)
        }
      }
      return
    }
    case IS_STORAGE_URL: {
      ...
      try {
        ...
        logger.error('Error replying to IS_STORAGE_URL: ', error.message, error)
      }
      return
    }
    case LISTEN_FOR_RCE_LOCK: {
      ...
      const unsubscribe = listenForRCELock(fileId, editorId, clientId, (result) => {
        try {
          ...
          logger.error('Error replying to LISTEN_FOR_RCE_LOCK: ', error.message, error)
        }
      })
      unsubscribeFunctions.set(messageId, unsubscribe)
      return
    ...
    case LISTEN_FOR_BACKUPS: {
      ...
      const unsubscribe = listenForBackups(userId, (result) => {
        try {
          ...
          logger.error('Error replying to LISTEN_FOR_BACKUPS: ', error.message, error)
        }
      })
      unsubscribeFunctions.set(messageId, unsubscribe)
      return
    ...
    case LISTEN_TO_CUSTOM_TEMPLATES: {
      ...
      const unsubscribe = listenToCustomTemplates(userId, (result) => {
        try {
          ...
          logger.error('Error replying to LISTEN_TO_CUSTOM_TEMPLATES: ', error.message, error)
        }
      })
      unsubscribeFunctions.set(messageId, unsubscribe)
      return

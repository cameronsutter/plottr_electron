...
export default function TemplatePicker ({startNew}) {
  ...
  const renderList = template => {
    ...
    return <li key={template.id} className={klasses} onClick={clickFunc}>{ template.name }</li>
  }
  ...
  const renderTemplateDetails = () => {
    ...
    let link = null
    if (selected.link) {
      link = <a className='template-picker__link' title={selected.link} onClick={() => shell.openExternal(selected.link)}><Glyphicon glyph='info-sign' /></a>
    }

    return <div className='panel panel-success'>
      <div className='panel-heading'>
        <h3 className='panel-title'>{selected.name}{link}</h3>
      </div>
      <div className='panel-body'>
        <p>{selected.description}</p>
        <Button bsSize='large' onClick={() => startNew(selected)}>{t('Create New Project')}</Button>
      </div>
    </div>
  }
  ...
  return <div className='dashboard__template-picker template-picker__wrapper'>

...
const FileLocationConnector = (connector) => {
  ...
  checkDependencies({ isTempFile, moveFromTemp, showItemInFolder, isMacOS, os })

  const FileLocation = ({ file, state, hasCurrentProLicense }) => {
    const [fileIsInTemp, setFileIsInTemp] = useState(false)

    useEffect(() => {
      isTempFile(file).then(setFileIsInTemp)
    }, [file])

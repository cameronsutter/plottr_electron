...
const ExportDialogConnector = (connector) => {
  ...
  const {
    platform: {
      log,
      dialog,
      export: { askToExport, setExportConfigSettings },
      isWindows,
    },
  } = connector
  checkDependencies({
  ...
  function ExportDialog(props) {
    ...
    const setType = (type) => setExportConfigSettings('savedType', type)
    ...
    const doExport = () => {
      ...
      askToExport(defaultPath, fullState, type, options[type], isWindows, (error, success) => {
        if (saveOptions) {
          setExportConfigSettings('savedType', type)

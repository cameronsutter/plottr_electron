...
const updateUI = (state, action) => {
  ...
  switch (action.type) {
    ...
    case PIN_PLOTLINE: {
      const currentPinnedPlotlines = parseInt(state.timeline.pinnedPlotlines)
      return {
        ...state,
        timeline: {
          ...state.timeline,
          pinnedPlotlines: Math.max(1, currentPinnedPlotlines + 1),
        },
      }
    }
    ...
    case UNPIN_PLOTLINE: {
      ...
      return {
        ...
        timeline: {
          ...state.timeline,
          pinnedPlotlines: Math.max(0, currentPinnedPlotlines - 1),
        },
      }
    }

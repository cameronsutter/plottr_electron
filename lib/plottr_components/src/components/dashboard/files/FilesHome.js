...
const FilesHomeConnector = (connector) => {
  ...
  const FilesHome = ({
    ...
    useEffect(() => {
      if (!isNewProjectModalVisible && template) {
        setTemplate(null)
      }
    }, [isNewProjectModalVisible])

    const startNamingTemplate = (template) => {
    ...
    const handleCreateNewProject = () => {
      projectActions.setShowNewProjectInputModal(true)
    }

    return (
      <div className="dashboard__files">
        <NewProjectInputModal template={template} />
        <NewFiles
          activeView={view}

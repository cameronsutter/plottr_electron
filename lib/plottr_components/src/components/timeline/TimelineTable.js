...
const TimelineTableConnector = (connector) => {
  ...
  class TimelineTable extends Component {
    ...
    handleReorderLines = (droppedPosition, originalPosition) => {
      ...
      const newPosition = isConflictWithPinned ? droppedPosition + 1 : droppedPosition
    ...
    handleTogglePinPlotline = (line) => {
      if (line?.isPinned) {
        this.props.lineActions.unPinPlotline(line.id)
      } else {
        const reorderedLines = reorderList(0, line.position, this.props.lines)
        this.props.lineActions.pinPlotline(line.id, reorderedLines, this.props.currentTimeline)
      }
    }

...
export function createNewAttribute(currentAtttributes, name, bookId) {
  // Characters
  const attributeExists = currentAtttributes.find((attr) => camelCase(attr.name) == camelCase(name))
  if (!attributeExists) {
    ...
    currentAtttributes.forEach((attr, idx) => {
      if (camelCase(attr.name) == camelCase(name)) {
        currentAtttributes[idx] = { ...attributeExists, ...values }
      }
    })

  ...
  describe('given a file with one level', () => {
    describe('and a template with one level', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(one_level_biased_top_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('plbda63b')))
          )
        })
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = one_level_biased_top_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
      ...
      describe('and the template is biased to the "middle"', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(one_level_biased_middle_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('plbda63b')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = one_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
      ...
      describe('and the template is biased to the "bottom"', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(one_level_biased_bottom_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('plbda63b')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = one_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
    ...
    describe('and a template with two levels', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(two_level_biased_top_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl7d013d')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = two_level_biased_top_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          expect(templateCards.length).toEqual(
            addedCards.length
          )
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
      ...
      describe('and the template is biased to the middle', () => {
        ...
        it('should have the cards in the template from the bottom level', () => {
          ...
          const cardsInBookFromTemplate = two_level_biased_middle_plotline_template.templateData.cards.filter((card) => {
            return bottomLevelBeatIds.indexOf(card.beatId) !== -1
          })
          expect(result.cards.map(withoutLineBeatOrCardIds)).toEqual(expect.arrayContaining(cardsInBookFromTemplate.map(withoutLineBeatOrCardIds)))
        })
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(two_level_biased_middle_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl7d013d')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = two_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
      ...
      describe('and the template is biased to the bottom', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(two_level_biased_bottom_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl7d013d')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = two_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
    ...
    describe('and a template with three levels', () => {
      describe('and the template is biased to the top', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(three_level_biased_top_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl8b2c85')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_top_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
      ...
      describe('and the template is biased to the middle', () => {
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(three_level_biased_middle_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl8b2c85')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_one_level.cards.map(({ id }) => id).indexOf(card.id) === -1
      ...
      describe('and the template is biased to the bottom', () => {
        ...
        const middleLevelBeats = tree.children(three_level_biased_bottom_plotline_template.templateData.beats['1'], topLevelBeats[0].id)
        const bottomLevelBeats = tree.children(three_level_biased_bottom_plotline_template.templateData.beats['1'], middleLevelBeats[0].id)
        it('should have more cards', () => {
          expect(result.cards.length).toBeGreaterThanOrEqual(file_with_one_level.cards.length)
        })
        it('should have the cards it used to have', () => {
        ...
        it('should have the cards in the template', () => {
          ...
          const cardsInBookFromTemplate = three_level_biased_bottom_plotline_template.templateData.cards.filter(
            ({ beatId }) => {
              return bottomLevelBeatIds.indexOf(beatId) !== -1
            }
          )
        ...
        it('should add the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(
            expect.arrayContaining(three_level_biased_bottom_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl8b2c85')))
          )
        })
        it('should have the plotlines that were there originally', () => {
          expect(result.lines).toEqual(expect.arrayContaining(file_with_one_level.lines))
  ...
  describe('given a file with two levels', () => {
    describe('and a template with one level', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateCards = one_level_biased_top_plotline_template.templateData.cards.filter(({ beatId }) => {
      ...
      describe('and the template is biased to the "middle"', () => {
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateBeats.map(({ id }) => id)
          const templateCards = one_level_biased_middle_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
    ...
    describe('and a template with two levels', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        const templateBottomBeats = templateTopBeats.flatMap((beat) => {
          return tree.children(two_level_biased_top_plotline_template.templateData.beats['1'], beat.id)
        })
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateCards = two_level_biased_top_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
      ...
      describe('and the template is biased to the middle', () => {
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateTopBeats.concat(templateBottomBeats).map(({ id }) => id)
          const templateCards = two_level_biased_middle_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
      ...
      describe('and the template is biased to the bottom', () => {
        ...
        const templateBottomBeats = templateTopBeats.flatMap((beat) => {
          return tree.children(two_level_biased_bottom_plotline_template.templateData.beats['1'], beat.id)
        })
        const resultTopBeats = tree.children(result.beats['1'], null)
        const resultBottomBeats = resultTopBeats.flatMap((beat) => {
          return tree.children(result.beats['1'], beat.id)
        ...
        it('should have the cards in the template', () => {
          expect(result.cards.map(withoutLineBeatOrCardIds)).toEqual(
            expect.arrayContaining(
              two_level_biased_bottom_plotline_template.templateData.cards.map(withoutLineBeatOrCardIds)
            )
          )
        })
        it('should have at least as many beats as the template at the top level', () => {
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateTopBeats.concat(templateBottomBeats).map(({ id }) => id)
          const templateCards = two_level_biased_bottom_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
    ...
    describe('and a template with three levels', () => {
      describe('and the template is biased to the top', () => {
        ...
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
          })
          const addedCards = result.cards.filter((card) => {
            return file_with_two_levels.cards.map(({ id }) => id).indexOf(card.id) === -1
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateCards = three_level_biased_bottom_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
      ...
      describe('and the template is biased to the middle', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(three_level_biased_middle_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl8b2c85'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateTopBeats.concat(templateMiddleBeats).map(({ id }) => id)
          const templateCards = three_level_biased_middle_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
      ...
      describe('and the template is biased to the bottom', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(three_level_biased_bottom_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl8b2c85'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateMiddleBeats.concat(templateBottomBeats).map(({ id }) => id)
          const templateCards = three_level_biased_bottom_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
  ...
  describe('given a file with three levels', () => {
    describe('and a template with one level', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(one_level_biased_top_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('plbda63b'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
      ...
      describe('and the template is biased to the "middle"', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(one_level_biased_middle_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('plbda63b'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = one_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id
      ...
      describe('and the template is biased to the "bottom"', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(one_level_biased_bottom_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('plbda63b'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = one_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateBeats.map(({ id }) => id)
          const templateCards = one_level_biased_bottom_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
    ...
    describe('and a template with two levels', () => {
      describe('and the template is biased to the "top"', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(two_level_biased_top_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl7d013d'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = two_level_biased_top_plotline_template.templateData.lines.map(({ id }) => {
            return id
      ...
      describe('and the template is biased to the middle', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(two_level_biased_middle_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl7d013d'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = two_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id
      ...
      describe('and the template is biased to the bottom', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(two_level_biased_bottom_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl7d013d'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = two_level_biased_bottom_plotline_template.templateData.lines.map(({ id }) => {
            return id
    ...
    describe('and a template with three levels', () => {
      describe('and the template is biased to the top', () => {
        ...
        it('should have the plotlines from the template', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(three_level_biased_top_plotline_template.templateData.lines.map(withoutIdsPositionsOrColor).map(withFromTemplateId('pl8b2c85'))))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_top_plotline_template.templateData.lines.map(({ id }) => {
            return id
        ...
        it('should insert cards on the plotlines from the template', () => {
          ...
          const templateBeatIds = templateTopBeats.concat(templateMiddleBeats).concat(templateBottomBeats).map(({ id }) => id)
          const templateCards = three_level_biased_top_plotline_template.templateData.cards.filter(({ beatId }) => {
            return templateBeatIds.indexOf(beatId) !== -1
          })
          expect(templateCards.length).toEqual(addedCards.length)
          expect(addedCards.map(withoutLineBeatOrCardIds)).toEqual(
      ...
      describe('and the template is biased to the middle', () => {
        ...
        it('should have the plotlines that were there originally', () => {
          expect(result.lines.map(withoutIdsPositionsOrColor)).toEqual(expect.arrayContaining(file_with_three_levels.lines.map(withoutIdsPositionsOrColor)))
        })
        it('should only add cards to the new plotlines', () => {
          const existingPlotlines = three_level_biased_middle_plotline_template.templateData.lines.map(({ id }) => {
            return id

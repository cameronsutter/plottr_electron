...
var MODULE_NAME_PATTERN = /(^|\/)plottr_locales$/
...
exports = module.exports = {
    ...
    return (
      ...
      MODULE_NAME_PATTERN.test(parent.source.value) && (
        node.type === 'ImportDefaultSpecifier' || (
          ...
          node.imported && node.imported.name === 't'
        )
      )
    )
  },
  ...
  isStringish: function (node) {
    return (
      this.isLiteralish(node) &&
      typeof this.getLiteralValue(node) === 'string'
    )
  },
  ...
  getLiteralValue: function (node) {
    ...
    switch (node.type) {
      ...
      case 'BinaryExpression':
        return (
          this.getLiteralValue(node.left) +
          this.getLiteralValue(node.right)
        )
      default:
        return node.value
    }
    ...
    var isDestructureRequire = (
      ...
      node.id.properties.some(function (property) {
        var isAHelper = (
          ...
          this.isHelperName(property.key.name)
        )

...
import { characterAttributeTabSelector, isSeriesSelector, selectedCharacterSelector } from '../selectors/ui'
...
const root = (dataRepairers) => (state, action) => {
  ...
  switch (action.type) {
    case CREATE_CHARACTER_ATTRIBUTE: {
      const newId = nextId(state.attributes)
      const bookId = characterAttributeTabSelector(state)
      const characterId = selectedCharacterSelector(state)
      const newAction = {
        ...action,
        newAttributeId: newId,
        bookId,
        characterId,
      }

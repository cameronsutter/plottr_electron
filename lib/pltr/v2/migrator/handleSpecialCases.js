...
const handleObjectTitlesOnCards = (file) => {
  const FIX_VERSION = 'm2022_1_11'

  const appliedMigrations =
    (file.file && file.file.appliedMigrations && file.file.appliedMigrations) || []

  if (appliedMigrations.indexOf(FIX_VERSION) > -1) {
    return file
  }

  const fixApplies = file.cards.some(
    (card) =>
      typeof card.title !== 'string' && isObject(card.title) && typeof card.title.title === 'string'
  )

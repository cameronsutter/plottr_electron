...
const NoteListViewConnector = (connector) => {
  ...
  class NoteListView extends Component {
    ...
    static detailID(notesByCategory, notes, categories, noteDetailId) {
      ...
      const allCategories = [...categories, { id: 0 }] // uncategorized
    ...
    updateFilter = (filter) => {
      ...
      this.props.uiActions.setNoteFilter({ filter })
    }

    clearFilter = () => {
      this.props.uiActions.setNoteFilter(null)
    }
    ...
    renderNotes() {
      ...
      categories.push({ id: 0, name: i18n('Uncategorized') })

...
const AccountConnector = (connector) => {
  ...
  const Account = ({
    ...
    const checkTrial = () => started && !expired && !hasLicense && !hasCurrentProLicense
    const checkTrialExpired = () => expired && !hasLicense && !hasCurrentProLicense
    const checkLicense = () => !!hasLicense
    ...
    }, [started, expired, hasLicense, hasCurrentProLicense])

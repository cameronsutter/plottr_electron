...
import { cloneDeep, keys, values, findLast, flattenDeep } from 'lodash'
...
function createNewNote(currentNotes, values) {
  ...
  if (!noteExist) {
    ...
    currentNotes.forEach((note, idx) => {
      if (note.name.toLowerCase().trim() == strippedNote) {
  ...
  const withNewCardsAndNewBeats = chapters
    ...
    .reduce((acc, chapter, beatKey) => {
            ...
            return withPlotlineSubFolderChildrenArray.reduce(
              (jsonWithPlotlineSubfolderCards, subfolder, idx) => {
                ...
                if (RTFContents && RTFContents.length) {
                  ...
                  fileContents.txtContent = createSlateParagraph(filterRTFContents(RTFContents))
                ...
                // add description arr filter sectionItems
      ...
      return beatChildrenWithoutStoryLinesArr.reduce((newJson, child, key) => {
        ...
        if (RTFContents && RTFContents.length) {
          const contents = getManuscriptSectionItems(newJson, RTFContents, bookId).filter(Boolean)
          fileContents.txtContent = createSlateParagraph(filterRTFContents(RTFContents))
...
function getManuscriptSectionItems(currentJson, RTFContents, bookId) {
  return RTFContents.map((item, idx) => {
    if (item && item.children) {
      const isBold = item.isBold
      const RTFText = item.children[0].text
      const isInlineTitle =
        RTFText.indexOf(':') && sectionItems.includes(RTFText.split(':')[0].toLowerCase().trim())
      const nextTextValue =
        RTFContents[idx + 1] &&
      ...
      if (isInlineTitle || isBold) {
        const strippedInlineTitleText =
          isInlineTitle && !isBold ? RTFText.split(':')[0].toLowerCase().trim() : ''
        ...
        const sectionText = isBold ? nextTextValue : isInlineTitle ? RTFText.split(':').pop() : null
        ...
        return {
          [strippedSectionName]: sectionItems.includes(strippedSectionName)
            ? sectionIds
            : sectionText,
        }
      } else return null
...
function filterRTFContents(RTFContents) {
  return RTFContents.filter((item, key) => {
    const hasText = !!item.children && !!item.children[0] && !!item.children[0].text
    if (hasText) {
      const strippedText = item.children[0].text.toLowerCase().trim()
      const strippedInlineText = item.children[0].text.split(':')[0].toLowerCase().trim()
      const prevText =
        RTFContents[key - 1] &&
        RTFContents[key - 1].children &&
        RTFContents[key - 1].children[0] &&
        RTFContents[key - 1].children[0].text
          ? RTFContents[key - 1].children[0].text.toLowerCase().trim()
          : ''
      return (
...
function getRTFContents(newJson, lines, rtf, matchId, bookId, isSection) {
...
function mergePairAttributes(attributesList) {

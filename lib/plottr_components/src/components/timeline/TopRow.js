...
const TopRowConnector = (connector) => {
  ...
  const TopRow = (props) => {
    ...
    const handleReorderLines = (droppedPosition, originalPosition) => {
      ...
      lineActions.reorderLines(droppedPosition, originalPosition, lines, currentTimeline)
    }

    const handleTogglePinPlotline = (line) => {
      const { lineActions, lines, currentTimeline } = props
      lineActions.togglePinPlotline(line, lines, currentTimeline)
    }

...
const FileActionsConnector = (connector) => {
  const {
    platform: {
      file: { deleteKnownFile, removeFromKnownFiles, renameFile, saveFile, copyFile },
      isMacOS,
      showItemInFolder,
      os,
    },
  ...
  const FileActions = ({
    ...
    const handleSaveToDesktop = (fileURL) => {
      copyFile(fileURL, 'desktop').then((file) => showItemInFolder(file))
    }
          ...
          {isInOfflineMode || isCloudFile || osIsUnknown ? null : (
            <Dropdown id={`file-action-${id}`} onSelect={doTheThing}>
              ...
              <Dropdown.Menu>
                ...
                {isCloudFile || osIsUnknown || missing ? null : (
                  <MenuItem eventKey="save-to-desktop">{t('Save to Desktop')}</MenuItem>
                )}

...
const CustomAttrFilterListConnector = (connector) => {
  ...
  class CustomAttrFilterList extends Component {
    ...
    renderFilterList(array, attr) {
      var items = orderBy(array, ['name', 'title', identity])
        .filter((i) => {
          return !Array.isArray(i)
        })
        .map((i) => this.renderFilterItem(i, attr))
      return (
  ...
  if (redux) {
    ...
    const chooseCustomAttributes = (state, type) => {
      switch (type) {
        ...
        case 'places':
          return state.present.customAttributes.places.filter(onlyText)
        case 'cards':
          return state.present.customAttributes.scenes.filter(onlyText)
        case 'notes':
          return state.present.customAttributes.notes.filter(onlyText)
        default:

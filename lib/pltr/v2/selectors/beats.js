...
import { currentTimelineSelector, isSeriesSelector, timelineViewIsTabbedSelector } from './ui'
import {
  maxDepth,
  nextId,
  beatsByPosition,
...
export const sparceBeatMap = createSelector(
  ...
  timelineViewIsTabbedSelector,
  (beats, beatHierarchyIsOn, timelineViewIsTabbed) =>
    visibleBeatsByPosition(beats, beatHierarchyIsOn, timelineViewIsTabbed).reduce(
      (acc, beat, index) => {
        acc[index] = beat.id
        return acc
      },

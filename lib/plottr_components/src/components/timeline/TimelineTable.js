...
const TimelineTableConnector = (connector) => {
  ...
  class TimelineTable extends Component {
    ...
    handleReorderLines = (droppedPosition, originalPosition) => {
      const isConflictWithPinned = this.props.lines.find(
        (line) => line.position == droppedPosition && line.isPinned
      )
      const isPinnedPlotline = this.props.lines.find(
        (line) => line.position == originalPosition && line.isPinned
      )

      const bothPinned = this.props.lines.find(
        (line) =>
          line.isPinned &&
      ...
      const newPosition =
        isConflictWithPinned && !bothPinned ? droppedPosition + 1 : droppedPosition
      const lines = reorderList(newPosition, originalPosition, this.props.lines)
      this.props.lineActions.reorderLines(lines, this.props.currentTimeline)
    }

    handleTogglePinPlotline = (line) => {
      const { pinnedPlotlines, lineActions, lines, currentTimeline } = this.props
      if (line?.isPinned) {
        const reorderedLines = reorderList(pinnedPlotlines - 1, line.position, lines)
        lineActions.unPinPlotline(line.id, reorderedLines, currentTimeline)
      } else {
        const reorderedLines = reorderList(0, line.position, lines)
        lineActions.pinPlotline(line.id, reorderedLines, currentTimeline)
      }
    }

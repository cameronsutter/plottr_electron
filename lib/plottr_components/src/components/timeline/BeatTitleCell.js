...
const BeatTitleCellConnector = (connector) => {
  class BeatTitleCell extends PureComponent {
    ...
    handleSetDraggedBeat = beat => {
      this.props.setDraggedBeat(beat)
    }
    ...
    handleDragStart = (e) => {
      ...
      this.handleSetDraggedBeat({level:this.props.hierarchyLevel.level,index:this.props.beatIndex})
    ...
    handleDragEnter = (e) => {
      ...
      const { dropDepth, dragging } = this.state
      const { beatIndex, draggedBeat, ui, hierarchyLevel } = this.props

      if (!dragging) this.setState({ dropDepth: dropDepth + 1 })
      
      this.setState({ showLine: true })
      let isParentLevel = draggedBeat.level - hierarchyLevel.level === 1
      let translation = ui.orientation === 'horizontal' ? SCENE_CELL_WIDTH : SCENE_CELL_HEIGHT
      if(beatIndex < draggedBeat.index) this.setState({previewLineOffset: -translation/2})
      if(beatIndex > draggedBeat.index || isParentLevel) this.setState({previewLineOffset: translation/2})
    }
    ...
    handleDrop = (e) => {
      ...
      if (droppedBeat.id == this.props.beat.id) {
        this.setState({ showLine: false })
        return
      }
        ...
        return (
          <Cell className="beat-table-cell">
            ...
            <div style={{transform: 'translateX(-20)'}}>
              <VisualLine
                tableLength={outerWidth * 2}
                orientation={this.props.ui.orientation === 'vertical' ? 'horizontal' : 'vertical'}
                color={hierarchyLevel.borderColor}
                isMedium={isMedium}
                offset={this.state.previewLineOffset}
              />
            </div>}
          </Cell>
        )
  ...
  BeatTitleCell.propTypes = {
    ...
    draggedBeat: PropTypes.object,
    setDraggedBeat: PropTypes.func,

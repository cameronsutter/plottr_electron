...
export default function OutlineOptions({
  ...
  const handleOnChangeLinesFilter = (lineId) => {
    if (!outlineFilter) {
      const filteredLines = lines.filter((item) => item.id !== lineId)
      setOutlineFilter(filteredLines.map((item) => item.id))
    } else {
      setOutlineFilter(lineId)
    }
  }
  ...
  const renderLines = () => {
    ...
    lines.map((line) => {
      const isChecked =
        (Array.isArray(outlineFilter) && outlineFilter.includes(line.id)) ||
        !Array.isArray(outlineFilter) ||
        !outlineFilter
      ...
      lineList.push(
        <li key={line.id}>
          <CheckOption
            checked={isChecked}
            onChange={() => handleOnChangeLinesFilter(line.id)}
            category="outline"
            attr="lines"
...
OutlineOptions.propTypes = {
  ...
  setOutlineFilter: PropTypes.func,
}

...
import { reduce, nextPositionInBook, nextId as nextBeatId } from '../helpers/beats'
...
const root = (dataRepairers) => (state, action) => {
  ...
  switch (action.type) {
    ...
    case ADD_LINES_FROM_TEMPLATE: {
      ...
      // ALSO:
      // TODO:
      // ignore beats unless there are > 1 hierarchyLevels
      // add cards to leaf nodes (beats) unless there are > 1 hierarchyLevels
      ...
      const beatPositionMap = Object.keys(beatTree.index).reduce((acc, id) => {
        acc[beatTree.index[id].position] = parseInt(id)
        return acc
      }, {})
      const cardToBeatIdMap = action.templateData.cards.reduce((acc, card) => {
        const beat = action.templateData.beats['1'].index[card.beatId]
        if (beatPositionMap[beat.position]) {
          // a beat in that position exists, so use it's id
          acc[card.id] = beatPositionMap[beat.position]
        } else {
      ...
      const levels = action.templateData.hierarchyLevels
      const ignoreBeats = !levels || (levels && Object.keys(levels).length < 2)

...
describe('2021.2.8', () => {
  ...
  describe('given a state with a chapter in it', () => {
    ...
    const { chapter } = initialState
    const card = {
      ...initialState.card,
      positionInChapter: initialState.card.positionInBeat,
      positionInBeat: undefined,
    }
    ...
    describe('and no beats', () => {
      ...
      it('should produce a state with that chapter as the only beat', () => {
        ...
        expect(migrated.beats).toEqual([chapter])
      })
    })
    ...
    describe('and cards which refer to that chapter', () => {
      ...
      it("should change the id of the card to point at the migrated chapter's id", () => {
        ...
        expect(migration(migrated).cards).toEqual([
          {
            ...{ ...card, positionInBeat: 0, positionInChapter: undefined },
            beatId: migrated.beats.length,
          },
        ])
      })
  ...
  describe('given a state with two chapters in it', () => {
    ...
    const chapters = [chapter, { ...chapter, id: 2, title: 'Another Chapter' }]
    const stateWithOneChapter = {
      ...newFile,
      chapters,
    }

...
const CardCellConnector = (connector) => {
  ...
  class CardCell extends Component {
    ...
    handleDragStart = (e) => {
      ...
      const cardsIdList = this.props.cards.map((card) => card.id)
      e.dataTransfer.setData('text/json', JSON.stringify({ cardId: cardsIdList }))
    }
    ...
    handleDrop = (e, allowDrop, cardIndex) => {
      ...
      console.log('drop', allowDrop, cardIndex)
      ...
      this.moveSceneCardAbove(droppedData.cardId, cardIndex)
    }
          ...
          <div key={card.id}>
            <ErrorBoundary>
              <Card
                ...
                handleDrop={(e) => this.handleDrop(e, allowDrop, idx)}

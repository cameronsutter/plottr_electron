...
const CHAPTER_NAMES = ['Act', 'Chapter', 'Scene']
const BEAT_NAMES = ['Act', 'Beat', 'Scene']

export function beatName(beats, beat) {
  const depth = tree.depth(beats, beat.id)
  const names = isSeries(beat) ? BEAT_NAMES : CHAPTER_NAMES
  return names[depth] || `Level-${depth}`
}

export function beatTitle(beats, beat, offset) {
  return beat.title == 'auto'
    ? i18n(`${beatName(beats, beat)} {number}`, { number: beat.position + offset + 1 })
    : beat.title
}

export function editingBeatLabel(beats, beat, offset) {
  return i18n(`${beatName(beats, beat)} {number} title`, { number: beat.position + offset + 1 })
}

export function beatPositionTitle(beats, beat, offset) {
  return i18n(`${beatName(beats, beat)} {number}`, { number: beat.position + offset + 1 })
}

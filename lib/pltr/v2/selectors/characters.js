...
import { characterAttributesForBookSelector } from './attributes'
...
export const displayedSingleCharacterSelector = createSelector(
  ...
  (character, bookId, attributeDescriptors) => {
    const currentBookAttributeDescriptors = attributeDescriptors.filter((attribute) => {
      return attribute.bookId === bookId
    })
    const currentBookAttributeDescirptorsById = mapValues(
      groupBy(currentBookAttributeDescriptors, 'id'),
      '0'
    )

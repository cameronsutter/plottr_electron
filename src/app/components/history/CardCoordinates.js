  ...
  renderDiff(diff, index) {
    ...
    return (
      <div key={`diff-${index}-${diff.path[1]}`}>
        ...
        <p>
          Before: <span className="history-component__item__before">{lhs.title}</span>
        </p>
        <p>
          After: <span className="history-component__item__after">{rhs.title}</span>
        </p>
      </div>
    )
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(CardCoordinates)

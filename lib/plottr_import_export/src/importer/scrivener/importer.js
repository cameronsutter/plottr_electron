...
export function getManuscriptSectionItems(currentJson, RTFContents, bookId) {
  ...
  return RTFContents.map((item, idx) => {
    if (item && item.children) {
      ...
      // Checks if the current text contains delimeter from `MANUSCRIPT_INLINE_CHAR_TO_SPLIT`
      // And the string before the delimeter is a section item
      const isInlineTitle =
...
function categorizePlot(sectionName) {
  if (
...
function isAPlotline(sectionName) {
    ...
    if (hasText) {
      ...
      const prevText =
        ...
        RTFContents[key - 1].children[0].text
          ...
          : ''
      return (
        !strippedText.includes(i18n('plot')) &&
        !sectionItemsWithPlotline.includes(strippedText) &&
        !sectionItemsWithPlotline.includes(strippedInlineText) &&

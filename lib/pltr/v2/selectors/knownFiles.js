...
import { getDateValue, isDeviceFileURL, withoutProtocol } from '../helpers/file'
...
export const flatSortedKnownFilesSelector = createSelector(
  ...
  (fileSystemFiles, cloudFiles, shouldBeInPro, isLoggedIn, searchTerm) => {
    ...
    const filteredFiles = files.filter((file) => {
      if (searchTerm && searchTerm.length > 1) {
        return withoutProtocol(file.fileURL).toLowerCase().includes(searchTerm.toLowerCase())
      } else {
        return true
      }
    })
...
export const fileURLSelector = (state, fileURL) => {
  return fileURL
}

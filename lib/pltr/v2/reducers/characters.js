    ...
    switch (action.type) {
      ...
      case CREATE_CHARACTER_ATTRIBUTE: {
        return state.map((character) => {
          ...
          const bookAttributes = attributes[action.bookId] || []
      ...
      case EDIT_CHARACTER_ATTRIBUTE_VALUE: {
        return state.map((character) => {
          if (character.id === action.characterId) {
            const attributes = character.attributes || {}
            const bookAttributes = attributes[action.bookId] || []
            const attributeExists = bookAttributes.find(({ id }) => {
              return id === action.attributeId
            })
            const attributesFromAll = attributes.all || []
            const allHasAttribute = attributesFromAll.some(({ id }) => {
              return id === action.attributeId
            })
            if (!attributeExists && !allHasAttribute) {
              return character
            }
            const bookId = attributeExists ? action.bookId : 'all'
            const targetAttributes = attributeExists ? bookAttributes : attributesFromAll

            return {
              ...character,
              attributes: {
                ...attributes,
                [bookId]: targetAttributes.map((attribute) => {
            ...
            attributes: {
              ...
              [bookId]: targetAttributes.map((attribute) => {
                if (attribute.id === action.id) {
                  return {
                    ...attribute,
                    value: firstParagraphText(attribute.value),
                  }
                }
      ...
      case DELETE_CHARACTER_ATTRIBUTE: {
        return state.map((character) => {
          ...
          return {
            ...
            attributes: {
              ...attributes,
              [bookId]: targetAttributes.filter((attribute) => {
